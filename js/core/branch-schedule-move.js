...
import {projectProductionSchedulerDb,projectProductionBranchAccessDb} from './production-role-view-projection.js';
...
export function mergeBranchMoveView(raw,timetable,branchIds,access={}){
 const scoped=projectProductionBranchAccessDb(raw,{...access,branchIds}),byId=new Map(timetable.lessons.map(row=>[row.id,row]));
 return{...scoped,lessons:scoped.lessons.filter(row=>byId.has(row.id)).map(row=>({...row,...byId.get(row.id)}))};
}

export async function createBranchScheduleMoveController({storage,locks,key,branchIds,access={},release,send,onApply,onState,createRequestId,initialDb,revision}){
 let raw=projectProductionBranchAccessDb(initialDb,{...access,branchIds}),rawRevision=revision,receiptRaw=null,latest=projectProductionSchedulerDb(raw),stopped=false;
 ...
 const queue=createProductionSchedulerQueue({storage,release,send:async request=>{const response=await send(request);receiptRaw=response?.branchDb?{db:response.branchDb,revision:response.sourceRecordRevision}:null;return response},createRequestId,maxChangesPerRequest:30,includeStudent:false,
  onApply:value=>{if(receiptRaw&&receiptRaw.revision===queue.diagnostics().sourceRecordRevision&&receiptRaw.revision>=rawRevision){raw=projectProductionBranchAccessDb(receiptRaw.db,{...access,branchIds});rawRevision=receiptRaw.revision;receiptRaw=null}latest=value;if(!stopped)onApply(mergeBranchMoveView(raw,value,branchIds,access))},onState});
 ...
 return Object.freeze({
  ...
  async acceptSnapshot(db,nextRevision){if(stopped||nextRevision<rawRevision||nextRevision<queue.diagnostics().sourceRecordRevision)return;raw=projectProductionBranchAccessDb(db,{...access,branchIds});rawRevision=nextRevision;await queue.acceptSnapshot(projectProductionSchedulerDb(raw),nextRevision);if(!stopped)onApply(mergeBranchMoveView(raw,latest,branchIds,access))},
  async move(db){
   ...
   const persisted=queue.queue(after,{scheduleAction:'lesson.move'});latest=after;onApply(mergeBranchMoveView(raw,latest,branchIds,access));
