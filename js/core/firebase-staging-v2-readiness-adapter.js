...
function valueOf(snapshot){if(snapshot==null)return null;if(typeof snapshot.exists==='function')return snapshot.exists()?snapshot.data():null;if(typeof snapshot.exists==='boolean'&&typeof snapshot.data==='function'){if(!snapshot.exists)return null;const value=snapshot.data();return value===undefined?null:value}return snapshot}
...
function assertOperationEpoch(entries,activationEpoch){for(const [,rows] of entries)for(const row of rows){const descriptor=Object.getOwnPropertyDescriptor(row.data,'activationEpoch');if(descriptor&&(!descriptor.enumerable||!Object.prototype.hasOwnProperty.call(descriptor,'value')||descriptor.value!==activationEpoch))throw new Error(STAGING_V2_READINESS_BLOCKER)}}
