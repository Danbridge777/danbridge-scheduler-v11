...
if(DANBRIDGE_ENVIRONMENT==='staging'){
 ...
 if(livePreflightParams.get('stagingLiveExecute')==='manual'&&livePreflightBackupId&&livePreflightRestoreDrillId&&livePreflightReadBudget&&livePreflightWriteBudget){let installed=false;const timer=setInterval(()=>{if(installed||cloudRole!=='owner')return;installed=true;clearInterval(timer);const button=document.createElement('button');button.id='stagingLiveOperationExecuteButton';button.type='button';button.className='btn';button.style.cssText='position:fixed;right:18px;bottom:92px;z-index:10002;background:#7f1d1d;color:#fff';button.textContent='手動執行 staging 逐筆遷移';button.onclick=async()=>{button.disabled=true;button.textContent='保存永久日誌並逐筆執行中…';try{if(!stagingLivePreflightEvidence)await prepareStagingLiveOperationPreflight({backupId:livePreflightBackupId,restoreDrillId:livePreflightRestoreDrillId,readBudget:livePreflightReadBudget,writeBudget:livePreflightWriteBudget,maxOperationsPerRun:livePreflightMaxOperations});const result=await executePreparedStagingLiveOperationPlan();document.body.dataset.stagingLiveExecutionResult=JSON.stringify({state:result.state,manifestHash:result.manifestHash,finalized:result.finalized,processed:result.processed,counts:result.counts,documentCount:result.documentCount,activeCount:result.activeCount,tombstoneCount:result.tombstoneCount,readTakeover:false,uploadOwnerStateAttached:false,productionAllowed:false});button.disabled=result.state==='complete';button.textContent=result.state==='complete'?`逐筆完成並讀回：${result.activeCount} 筆`:`已安全暫停：${result.counts.confirmed}/${result.counts.total}，按此續傳`}catch(error){button.disabled=false;button.textContent='執行已阻擋，按此重試';cloudStatus(String(error?.message||error),'error')}};document.body.appendChild(button)},200)}
 const liveResumeManifestHash=livePreflightParams.get('stagingLiveResume');if(liveResumeManifestHash){let installed=false;const timer=setInterval(()=>{if(installed||cloudRole!=='owner')return;installed=true;clearInterval(timer);const button=document.createElement('button');button.id='stagingLiveOperationResumeButton';button.type='button';button.className='btn';button.style.cssText='position:fixed;right:18px;bottom:92px;z-index:10002;background:#7f1d1d;color:#fff';button.textContent='續傳 staging 永久操作日誌';button.onclick=async()=>{button.disabled=true;button.textContent='核對 manifest 與日誌後續傳中…';try{const result=await resumeStagingLiveOperationPlan(liveResumeManifestHash);document.body.dataset.stagingLiveExecutionResult=JSON.stringify({state:result.state,manifestHash:result.manifestHash,finalized:result.finalized,processed:result.processed,counts:result.counts,documentCount:result.documentCount,activeCount:result.activeCount,tombstoneCount:result.tombstoneCount,readTakeover:false,uploadOwnerStateAttached:false,productionAllowed:false});button.disabled=result.state==='complete';button.textContent=result.state==='complete'?`續傳完成並讀回：${result.activeCount} 筆`:`已安全暫停：${result.counts.confirmed}/${result.counts.total}，按此續傳`}catch(error){button.disabled=false;button.textContent='續傳已阻擋，按此重試';cloudStatus(String(error?.message||error),'error')}};document.body.appendChild(button)},200)}
}
