...
function rememberedPitrPreviewDatabaseId(){const urlValue=new URLSearchParams(location.search).get('pitrPreviewDatabase')||'',stored=sessionStorage.getItem('danbridgePitrPreviewDatabaseId')||'',value=String(urlValue||stored).trim();return /^pitr-preview-[a-z0-9-]{12,63}$/.test(value)?value:''}
function rememberPitrPreviewDatabaseId(databaseId){activePitrPreviewDatabaseId=databaseId;if(databaseId)sessionStorage.setItem('danbridgePitrPreviewDatabaseId',databaseId);else sessionStorage.removeItem('danbridgePitrPreviewDatabaseId')}
