...
/* Daniel 2026-08 recovery completed on 2026-08-13. One-time write tools were removed; keep only the generic read-only inventory below. */
/*
...
async function applyConfirmedRecoveryAsOwner(file){
 if(cloudRole!=='owner')throw new Error('只有 Owner 可執行原子批次恢復');const text=await file.text(),hash=await sha256Text(text);if(hash!==CONFIRMED_AA_RECOVERY_SHA256)throw new Error('救援檔案 SHA-256 不符，已阻止寫入');const report=JSON.parse(text),rows=report.missingLessons||[],students=report.missingStudents||[],wilson=students.find(x=>x.id==='ent_fec1fa6e-0a08-4602-ac0c-1a73c077c64a');if(report.readOnly!==true||rows.length!==67||students.length!==1||!wilson||wilson.name!=='Wilson'||rows.some(x=>x.teacherId!=='mrlilwcujyemtb'||x.date<'2026-08-01'||x.date>'2026-08-16'||!x.id||!x.studentId)||new Set(rows.map(x=>x.id)).size!==67)throw new Error('救援內容未通過固定範圍驗證');if(!confirm('Owner 將以一筆原子交易補入正式主資料缺少的 Daniel 67 堂與 Wilson。\n\n已存在課程 ID 自動跳過；不覆蓋其他課程。確定執行？'))return;
 const mainRef=doc(cloud,'companies',COMPANY_ID,'data','main'),result=await runTransaction(cloud,async transaction=>{const mainSnap=await transaction.get(mainRef);if(!mainSnap.exists())throw new Error('正式主資料不存在');const before=deepCopy(mainSnap.data()?.db),after=deepCopy(before),lessonIds=new Set((after.lessons||[]).map(x=>String(x.id))),studentIds=new Set((after.students||[]).map(x=>String(x.id)));if(!studentIds.has(wilson.id)){after.students.push({...schedulerSafeStudent(wilson),billing:'hour',rate:0,note:''});studentIds.add(wilson.id)}let added=0,skipped=0;for(const row of rows){const lesson=schedulerSafeLesson(row.raw||row);if(lessonIds.has(lesson.id)){skipped++;continue}if(!studentIds.has(String(lesson.studentId)))throw new Error(`缺少學生 ${lesson.studentId}，已阻止整批寫入`);if(!lessonTeacherIds(lesson).every(id=>(after.teachers||[]).some(t=>String(t.id)===id)))throw new Error(`缺少老師 ${lesson.teacherId}，已阻止整批寫入`);after.lessons.push({...lesson,paymentStatus:'unpaid',chargeStudent:'yes',payTeacher:'yes',note:''});lessonIds.add(lesson.id);added++}const detail=buildImmutableDataAudit(before,after);detail.eventId='confirmed-daniel-recovery-20260801-20260816';detail.action='confirmed-schedule-recovery';const audit=immutableAuditRecord(detail),auditSnap=await transaction.get(audit.ref),finalHash=dataHash(after);transaction.set(mainRef,{db:after,updatedAt:serverTimestamp(),updatedBy:cloudUid,clientHash:finalHash},{merge:false});if(!auditSnap.exists())transaction.set(audit.ref,audit.payload);for(const row of rows)transaction.set(doc(cloud,'companies',COMPANY_ID,'scheduleRequests',`confirmed-recovery-${row.id}`),{status:'applied',appliedAt:serverTimestamp(),appliedBy:cloudUid},{merge:true});return{added,skipped,after,finalHash}});applyingCloud=true;window.__danbridgeSetDB(deepCopy(result.after));persistCurrentLocalView();window.renderAll?.();applyingCloud=false;lastUploadedHash=result.finalHash;lastCloudSnapshotHash=result.finalHash;lastPublishedOwnerDB=deepCopy(result.after);ownerBaselineReady=true;publishRoleViewsWithRetry(result.after);cloudStatus(`原子恢復完成：新增 ${result.added} 堂、已存在跳過 ${result.skipped} 堂`,'ok');alert(`原子恢復完成。\n新增 ${result.added} 堂，已存在跳過 ${result.skipped} 堂。`)
}
function installOwnerConfirmedRecoveryButton(){if(cloudRole!=='owner'||document.getElementById('ownerConfirmedRecoveryButton'))return;const button=document.createElement('button');button.id='ownerConfirmedRecoveryButton';button.type='button';button.className='btn';button.style.cssText='position:fixed;right:18px;bottom:92px;z-index:10003;background:#7f1d1d;color:#fff;border-color:#fecaca';button.textContent='Owner 原子補回確認的67堂';button.onclick=()=>{const input=document.createElement('input');input.type='file';input.accept='.json,.txt,application/json,text/plain';input.onchange=async()=>{try{if(input.files?.[0])await applyConfirmedRecoveryAsOwner(input.files[0])}catch(e){console.error(e);cloudStatus('Owner 原子恢復失敗：'+(e.message||e),'error');alert('Owner 原子恢復失敗：'+(e.message||e))}};input.click()};document.body.appendChild(button)}
*/
