...
import {readRoleViewForAudit} from './role-view-audit-reader.js?v=20.26.367';
import {projectProductionBranchAccessDb} from './production-role-view-projection.js?v=20.26.367';
import {buildAccessPreset,isSchedulerAccess,listAccessPresets} from './access-presets.js?v=20.26.367';
...
import {createGoogleSignInFlow} from './google-sign-in-flow.js?v=20.26.367';
import {installAppCheckExchangeObserver} from './app-check-exchange-observer.js?v=20.26.367';
import {createLessonReportClient} from './lesson-report-client.js?v=20.26.367';
import {createTeacherReportHydrator} from './teacher-report-hydrator.js?v=20.26.367';
...
import {bootstrapDanbridgeFirebase} from './firebase-environment-bootstrap.js?v=20.26.367';
import {createScheduleNotificationPresenter,teacherLeaveNotificationStatusLabel} from './schedule-notification-presentation.js?v=20.26.367';
import {scheduleNotificationReadFilters,scheduleNotificationMatchesFilters} from './schedule-notification-read-scope.js?v=20.26.367';
import {createShardedSnapshot,assembleShardedSnapshot,canRunStagingShadow} from './cloud-sharded-store.js?v=20.26.367';
import {createFirebaseRecordShadowAdapter} from './firebase-record-shadow-adapter.js?v=20.26.367';
import {createFirebaseFullRecordShadowAdapter} from './firebase-full-record-shadow-adapter.js?v=20.26.367';
import {buildRecordShadowRunManifest,verifyRecordShadowRun,buildRecordShadowActivation,canonicalRecordShadowCore,canonicalLegacyRecordShadowCore,extractFullRecordShadowSyncResult,buildFullRecordShadowRunIdentity} from './cloud-record-shadow-run.js?v=20.26.367';
import {evaluateRecordShadowReadCandidate} from './cloud-record-shadow-read-candidate.js?v=20.26.367';
import {prepareImmutableMigrationBackup,verifyImmutableMigrationBackupReadback,sealImmutableMigrationBackup,verifyImmutableMigrationBackupManifest,sha256Canonical} from './cloud-immutable-migration-backup.js?v=20.26.367';
import {createFirebaseRoleViewCandidateAdapter} from './firebase-role-view-candidate-adapter.js?v=20.26.367';
import {verifyOwnRoleViewCandidateReadback} from './cloud-role-view-candidate.js?v=20.26.367';
import {buildFullRecordCandidateManifest,buildRoleViewCandidateManifest as buildLegacyRoleViewCandidateManifest,buildAtomicRecordActivation,evaluateAtomicRecordActivation} from './cloud-record-activation.js?v=20.26.367';
import {decideRecordReadTakeover} from './cloud-record-read-takeover.js?v=20.26.367';
import {FULL_RECORD_COLLECTIONS,rebuildFullRecordShadowDb} from './cloud-full-record-shadow.js?v=20.26.367';
import {recordDataDigest,recordDataHash} from './cloud-record-data-hash.js?v=20.26.367';
import {buildStagingLivePreflight} from './cloud-staging-live-preflight.js?v=20.26.367';
import {createBrowserOperationJournalStorage} from './browser-operation-journal-storage.js?v=20.26.367';
import {createOwnerDraftStore} from './cloud-owner-draft-store.js?v=20.26.367';
import {createProductionSchedulerQueue,acquireProductionSchedulerLease} from './production-scheduler-queue.js?v=20.26.367';
import {createBrowserStagingLiveExecutionStorage} from './browser-staging-live-execution-storage.js?v=20.26.367';
import {createOperationJournal} from './cloud-operation-journal.js?v=20.26.367';
import {enqueueOperationPlan,runOperationWorker} from './cloud-operation-worker.js?v=20.26.367';
import {createFirebaseLiveRecordOperationAdapter} from './firebase-live-record-operation-adapter.js?v=20.26.367';
import {assertStagingExecutionManifestEnvelope,stripStagingExecutionManifestAudit,verifyStagingLiveJournalRows} from './cloud-staging-live-activation.js?v=20.26.367';
import {createFirebaseStagingLiveActivationAdapter} from './firebase-staging-live-activation-adapter.js?v=20.26.367';
import {createActiveRecordPageController} from './cloud-active-record-page-controller.js?v=20.26.367';
import {createFirebaseActiveRecordStreamAdapter} from './firebase-active-record-stream-adapter.js?v=20.26.367';
import {createFirebaseRoleRecordViewAdapter} from './firebase-role-record-view-adapter.js?v=20.26.367';
import {createFirebaseRoleRecordStreamAdapter} from './firebase-role-record-stream-adapter.js?v=20.26.367';
import {createActiveRoleRecordPublishQueue} from './cloud-active-role-record-publish-queue.js?v=20.26.367';
import {decideOwnerActiveSaveIntent} from './cloud-owner-active-save-intent.js?v=20.26.367';
import {createRecordSyncActiveFailureResume} from './record-sync-active-failure-resume.js?v=20.26.367';
import {cloudBackupListingState,dailyBackupChunkCore,prepareDailyShardedBackup,sealDailyShardedBackup,verifyDailyShardedBackupReadback} from './cloud-daily-sharded-backup.js?v=20.26.367';
import {createFirebaseRecordSyncCandidateAdapter} from './firebase-record-sync-candidate-adapter.js?v=20.26.367';
import {buildRecordSyncRoleEvidence,RECORD_SYNC_ROLE_SCENARIOS} from './cloud-record-sync-role-evidence.js?v=20.26.367';
import {buildRecordSyncActivationManifest,buildActiveRecordSyncControl,evaluateActiveRecordSyncControl} from './cloud-record-sync-control.js?v=20.26.367';
import {createFirebaseRecordSyncActivationAdapter} from './firebase-record-sync-activation-adapter.js?v=20.26.367';
...
import {loadProfileAfterAuthReady} from './cloud-auth-profile-bootstrap.js?v=20.26.367';
import {RECORD_SYNC_V2_TAKEOVER_CANDIDATE_CONTROL_PATH,RECORD_SYNC_V2_TAKEOVER_CANDIDATE_HEAD_PATH,createFirebaseRecordSyncV2TakeoverCandidateAdapter} from './firebase-record-sync-v2-takeover-candidate-adapter.js?v=20.26.367';
import {createStagingV2AuthorityReadLoader} from './staging-v2-authority-read-loader.js?v=20.26.367';
import {createStagingV2AuthoritySaveBrowserClient} from './staging-v2-authority-save-browser-client.js?v=20.26.367';
import {createStagingV2ActiveRecordOperationSender,normalizeStagingV2FirestoreValue,stagingV2H0GenesisBaselineDocuments} from './staging-v2-active-record-browser-bridge.js?v=20.26.367';
import {createFirebaseProductionRecordConflictAdapter,createFirebaseProductionRecordStreamAdapter} from './firebase-production-record-runtime-adapter.js?v=20.26.367';
import {buildProductionRecordRuntimeControl,assertProductionRecordRuntimeControl,buildProductionRecordRuntimeSafety,assertProductionRecordRuntimeSafety,assertLegacyProductionRecordRuntimeSafety} from './cloud-production-record-runtime.js?v=20.26.367';
import {CLOUD_BOOTSTRAP_STAGES,createCloudBootstrapProgress} from './cloud-bootstrap-progress.js?v=20.26.367';
import {createProductionTrustedOperationClient} from './production-trusted-operation-client.js?v=20.26.367';
import {prepareActiveRecordSync} from './cloud-active-record-sync.js?v=20.26.367';
import {createFirebaseCallableHttpClient} from './firebase-callable-http-client.js?v=20.26.367';
import {createLimitedUseAppCheckTokenPool,takeFreshLimitedUseAppCheckToken} from './app-check-limited-use-token.js?v=20.26.367';
import {createPublishedRoleViewConsumer} from './published-role-view-consumer.js?v=20.26.367';
import {createFirestoreRolePartBatchReader} from './firestore-role-part-batch-reader.js?v=20.26.367';
import {createPublishedSchedulerReceiptReader,PUBLISHED_SCHEDULER_RESPONSE_SCHEMA} from './published-scheduler-receipt.js?v=20.26.367';
...
function purgeRetiredFirestoreWebStorage(){
 const retired=/^firestore_(?:clients|mutations|targets|sequence_number|bundle_loaded|zombie)_/;
 let removed=0;
 try{
  const keys=[];
  for(let index=0;index<localStorage.length;index++){const key=localStorage.key(index);if(key&&retired.test(key))keys.push(key)}
  for(const key of keys){localStorage.removeItem(key);removed++}
  document.body.dataset.retiredFirestoreCache=`cleared:${removed}`;
 }catch{document.body.dataset.retiredFirestoreCache='unavailable'}
 return removed
}
purgeRetiredFirestoreWebStorage();
...
const APP_RELEASE='20.26.367';
// Kept only for repairing requests created by the retired email-bound client.
// Current authorization is exclusively role/capability based.
const LEGACY_SCHEDULER_ACCOUNT_EMAILS=new Set(['aa0966626336@gmail.com']);
...
installAppCheckExchangeObserver({target:globalThis,projectId:firebaseConfig.projectId,appId:firebaseConfig.appId});
...
const scopedPayrollCall=!publishedWorkspace&&(DANBRIDGE_ENVIRONMENT==='production'&&productionAppCheck?httpsCallable(productionFunctions,'productionReadScopedPayroll',{limitedUseAppCheckTokens:true}):DANBRIDGE_ENVIRONMENT==='staging'&&stagingV2AppCheck?httpsCallable(stagingFunctions,'stagingReadScopedPayroll',{limitedUseAppCheckTokens:true}):null);
window.__danbridgeReadScopedPayroll=async payload=>{
 const user=auth.currentUser;
 if(!scopedPayrollCall||!user||user.uid!==cloudUid||String(user.email||'').toLowerCase()!==cloudEmailKey)throw Error('財務核對登入尚未就緒');
 const result=await scopedPayrollCall(payload);
 if(auth.currentUser!==user)throw Error('登入身分已變更，未採用舊帳號金額');
 return result.data;
};
const bookPurchaseCall=!publishedWorkspace&&(DANBRIDGE_ENVIRONMENT==='production'&&productionAppCheck?httpsCallable(productionFunctions,'productionBookPurchase',{limitedUseAppCheckTokens:true}):DANBRIDGE_ENVIRONMENT==='staging'&&stagingV2AppCheck?httpsCallable(stagingFunctions,'stagingBookPurchase',{limitedUseAppCheckTokens:true}):null);
window.__danbridgeBookPurchaseCall=async payload=>{
 const user=auth.currentUser;
 if(!bookPurchaseCall||!user||user.uid!==cloudUid||String(user.email||'').toLowerCase()!==cloudEmailKey)throw Error('書籍代購登入尚未就緒');
 const result=await bookPurchaseCall(payload);
 if(auth.currentUser!==user)throw Error('登入身分已變更，請重新登入確認');
 return result.data;
};
...
async function takeStagingNotificationLimitedUseToken(){
 return takeFreshLimitedUseAppCheckToken({appCheck:stagingV2AppCheck,getLimitedUseToken,missingMessage:'staging notification limited-use App Check token missing',unavailableMessage:'staging notification App Check unavailable'});
}
const productionSchedulerLimitedUseTokenPool=createLimitedUseAppCheckTokenPool({appCheck:productionAppCheck,getLimitedUseToken,missingMessage:'production scheduler limited-use App Check token missing',unavailableMessage:'production scheduler App Check unavailable'});
function warmProductionSchedulerLimitedUseToken(){return productionSchedulerLimitedUseTokenPool.warm()}
async function takeProductionSchedulerLimitedUseToken(){return productionSchedulerLimitedUseTokenPool.take()}
async function takeProductionNotificationLimitedUseToken(){
 return takeFreshLimitedUseAppCheckToken({appCheck:productionAppCheck,getLimitedUseToken,missingMessage:'production notification limited-use App Check token missing',unavailableMessage:'production notification App Check unavailable'});
}
function warmStagingV2TokenForScheduleIntent(event){
 if(!['staging','production'].includes(DANBRIDGE_ENVIRONMENT))return;
 if(event.type==='keydown'&&!['Enter',' '].includes(event.key))return;
 const target=event.target;
 if(!(target instanceof Element)||!target.closest('#calendar,#lessonModal,#batchModal,#smartSchedulerModal'))return;
 if(DANBRIDGE_ENVIRONMENT==='staging')warmStagingV2LimitedUseToken()?.catch(()=>{});else warmProductionSchedulerLimitedUseToken()?.catch(()=>{});
}
document.addEventListener('pointerdown',warmStagingV2TokenForScheduleIntent,{capture:true,passive:true});
document.addEventListener('keydown',warmStagingV2TokenForScheduleIntent,true);
...
async function verifyActiveStagingV2Owner(user){
 const uid=user?.uid,email=typeof user?.email==='string'?user.email.trim().toLowerCase():'';
 const assertCurrent=()=>{
  if(DANBRIDGE_ENVIRONMENT!=='staging'||firebaseConfig.projectId!=='danbridge-d8877-staging'||app.options?.projectId!=='danbridge-d8877-staging'||auth.app!==app||cloud.app!==app||auth.currentUser!==user||auth.currentUser?.uid!==uid||String(auth.currentUser?.email||'').trim().toLowerCase()!==email||typeof uid!=='string'||!/^[A-Za-z0-9_.:-]{8,128}$/.test(uid)||!email||cloudRole!=='owner')throw new Error('V2 authority requires the same active staging Owner');
 };
 assertCurrent();
 if(email!==OWNER_EMAIL){
  const snapshot=await getDocFromServer(doc(cloud,'companyAccess',email));
  assertCurrent();
  const access=snapshot.exists()?snapshot.data():null;
  if(access?.active!==true||access.companyId!==COMPANY_ID||access.role!=='owner')throw new Error('V2 authority requires an active company Owner access record');
 }
 return {uid,email};
}
export function createExplicitStagingV2AuthorityReadLoader(){
 ...
 return Object.freeze({scope:loader.scope,async load(request){
  ...
  if(typeof user?.getIdTokenResult!=='function')throw new Error('V2 authority reader requires a signed-in Owner token');
  await verifyActiveStagingV2Owner(user);
...
let branchMoveController=null,branchMoveInit=null,branchMoveGeneration=0,branchMoveRetryTimer=null;
window.__danbridgeBranchMoveReady=()=>!!branchMoveController&&window.DanbridgeAccess?.getContext?.().canMoveSchedule===true;
let schedulerBaselineLessons=[],schedulerBaselineStudents=[];
let productionSchedulerQueue=null,productionSchedulerQueueInit=null,productionSchedulerViewChain=Promise.resolve(),productionSchedulerGeneration=0,productionSchedulerLease=null;
...
let ownerRecoveryPersistenceTimer=null;
let ownerRecoveryPersistenceIdle=null;
...
onAuthStateChanged(auth,()=>window.__danbridgeSetBookIdentity?.(null));
...
function persistOwnerSyncRecovery(){
 ...
 if(localDirtyHash.startsWith('schedule-pending:')||localDirtyHash.startsWith('student-pending:'))localDirtyHash=dataHash(window.__danbridgeGetDB?.());
...
function scheduleOwnerSyncRecoveryPersistence(version){
 if(ownerRecoveryPersistenceTimer!==null)clearTimeout(ownerRecoveryPersistenceTimer);
 if(ownerRecoveryPersistenceIdle!==null&&typeof cancelIdleCallback==='function'){cancelIdleCallback(ownerRecoveryPersistenceIdle);ownerRecoveryPersistenceIdle=null}
 ownerRecoveryPersistenceTimer=setTimeout(()=>{ownerRecoveryPersistenceTimer=null;const persist=()=>{ownerRecoveryPersistenceIdle=null;if(version!==localMutationVersion||!localDirtyHash)return;persistOwnerSyncRecovery()};if(typeof requestIdleCallback==='function')ownerRecoveryPersistenceIdle=requestIdleCallback(persist,{timeout:1200});else persist()},700);
}
function clearOwnerSyncRecovery(){if(ownerRecoveryPersistenceTimer!==null){clearTimeout(ownerRecoveryPersistenceTimer);ownerRecoveryPersistenceTimer=null}if(ownerRecoveryPersistenceIdle!==null&&typeof cancelIdleCallback==='function'){cancelIdleCallback(ownerRecoveryPersistenceIdle);ownerRecoveryPersistenceIdle=null}ownerRecoveryBaseDB=null;try{localStorage.removeItem(OWNER_SYNC_RECOVERY_KEY)}catch{}}
...
function renderCloudBootstrapProgress(snapshot){
 let panel=document.getElementById('cloudBootstrapProgress');if(!panel){panel=document.createElement('div');panel.id='cloudBootstrapProgress';panel.setAttribute('role','status');panel.setAttribute('aria-live','polite');panel.style.cssText='position:fixed;left:12px;bottom:58px;z-index:10000;width:min(360px,calc(100vw - 24px));box-sizing:border-box;overflow-wrap:anywhere;padding:12px 14px;border-radius:14px;background:#fff;color:#172033;border:1px solid #dbe4ef;box-shadow:0 12px 30px rgba(15,23,42,.18);font-size:12px';document.body.appendChild(panel)}
...
function failCloudBootstrap(error,{readOnly=false}={}){if(!cloudBootstrapProgress)return;clearTimeout(cloudBootstrapTimeout);cloudBootstrapTimeout=null;renderCloudBootstrapProgress(cloudBootstrapProgress.fail(error,{readOnly}))}
...
const COMPANY_ACCESS_MUTABLE_FIELDS=['email','displayName','role','companyId','active','invitedBy','teacherId','teacherName','managerName','branchIds','branchNames','readOnly','canSubmitOwnReports','canManageSchedule','canMoveSchedule','hideFinancials','canViewBranchFinance','scheduleBranchIds','scopedDb'];
const COMPANY_ACCESS_USER_MIRROR_FIELDS=['displayName','role','active','teacherId','teacherName','managerName','branchIds','branchNames','readOnly','canSubmitOwnReports','canManageSchedule','canMoveSchedule','hideFinancials','canViewBranchFinance','scheduleBranchIds','scopedDb'];
...
function assertStagingAccessProjectionUnchanged(current,next){
 ...
 const projection=value=>JSON.stringify({role:value.role||'',teacherId:value.teacherId||'',branchIds:[...(value.branchIds||[])].sort(),scheduleBranchIds:[...(value.scheduleBranchIds||[])].sort(),hideFinancials:value.hideFinancials===true,canViewBranchFinance:value.canViewBranchFinance===true,canManageSchedule:value.canManageSchedule===true});
...
async function ensureProfile(user){
 ...
 return {
   ...
   canViewBranchFinance:a.canViewBranchFinance===true,
...
async function recordSuccessfulLogin(user,profile){
 // Staging V2 intentionally exposes users/* as read-only. Login telemetry must
 // never reopen a browser write path or generate a false operational warning.
 if(DANBRIDGE_ENVIRONMENT==='staging')return;
...
async function listCloudSafetyBackups(){
 ...
 try{
  ...
  const stateOf=x=>cloudBackupListingState(x,{day:x?.id,environment:DANBRIDGE_ENVIRONMENT,legacyHash:dataHash});
  list.innerHTML=rows.length?rows.map(x=>{const state=stateOf(x),legacyVerified=state==='legacy-verified',shardedVerified=state==='sealed',verified=legacyVerified||shardedVerified,time=x.verifiedAt||x.createdAt;return`<div class="backup-item"><div class="info"><b>${escapeHTML(x.id)}</b><div class="small">學生 ${x.counts?.students||0}｜老師 ${x.counts?.teachers||0}｜課程 ${x.counts?.lessons||0}${shardedVerified?`<br>分片 ${x.chunkCount} 份｜${x.recordCount} 筆`:''}<br>${escapeHTML(formatNotificationTimestamp(time)||'時間確認中')}</div></div><div class="row-actions"><span class="pill ${verified?'green':'red'}">${shardedVerified?'已封存・還原前重驗':legacyVerified?'雜湊一致':state==='uploading'?'建立中・尚未驗證':'驗證失敗'}</span>${verified?`<button type="button" class="btn cloud-backup-restore" data-day="${escapeHTML(x.id)}">還原</button>`:''}</div></div>`}).join(''):'<span class="small">尚未建立雲端安全快照。</span>';
  list.querySelectorAll('.cloud-backup-restore').forEach(button=>button.onclick=()=>restoreCloudSafetyBackup(button.dataset.day));
  const today=rows.find(x=>x.id===backupDayKey()),state=stateOf(today),partial=shardedResult.status==='rejected'||legacyResult.status==='rejected';
  dailyBackupConfirmedDay=!partial&&state==='sealed'?today.id:'';
  if(partial)resilienceStatus('部分雲端快照清單讀取失敗，尚未完整確認','error');
  else if(state==='sealed'||state==='legacy-verified')resilienceStatus(state==='sealed'?'今日雲端快照已封存；還原前將重新驗證':'今日雲端快照雜湊已確認','ok');
  else resilienceStatus(state==='missing'?'今日尚未建立雲端快照':state==='uploading'?'今日雲端快照建立中，尚未驗證':'今日雲端快照驗證失敗',state==='invalid'?'error':'pending');
 }catch(e){dailyBackupConfirmedDay='';console.error('listCloudSafetyBackups failed',e);resilienceStatus('雲端快照清單讀取失敗','error');list.innerHTML='<span class="small">讀取失敗，請稍後重試。</span>'}
}
...
function backendHealthFreshness(data,nowMs=Date.now()){
 const checkedMs=data?.checkedAt?.toMillis?.();
 if(data?.schema!=='danbridge-production-owner-health-v1'||!Number.isFinite(checkedMs)||checkedMs<=0)return{state:'missing',checkedAt:'',ageMs:null};
 const ageMs=nowMs-checkedMs;
 const maxAgeMs=data.maxAgeMs===undefined?36*3600000:Number.isSafeInteger(data.maxAgeMs)&&data.maxAgeMs>=60000&&data.maxAgeMs<=36*3600000?data.maxAgeMs:0;
 return{state:!maxAgeMs||ageMs< -300000?'invalid':ageMs>maxAgeMs?'stale':'current',checkedAt:new Date(checkedMs).toISOString(),ageMs:Math.max(0,ageMs)};
}
...
function buildSyncHealthReport({db,mainData,pendingRequests,errorRows,maintenanceData=null,ownerHealthData=null,restoreRehearsalData=null}){
 ...
 const alerts=[];
 const healthFreshness=backendHealthFreshness(ownerHealthData);
 if(DANBRIDGE_ENVIRONMENT==='production'&&healthFreshness.state!=='current')alerts.push({level:'pending',message:healthFreshness.state==='stale'?'後端健康檢查已超過有效期限；下列為歷史結果，不能代表目前狀態。':healthFreshness.state==='invalid'?'後端健康檢查時間異常，尚不能確認目前狀態。':'尚無有效的後端健康檢查時間，不能宣稱目前正常。'});
 ...
 const capacity=ownerHealthData?.roleCapacity;
 if(capacity?.schema==='danbridge-role-capacity-v1'&&Number.isSafeInteger(capacity.maximumBytes)&&capacity.maximumBytes>=560000&&capacity.maximumBytes<720000&&capacity.budgetBytes===800000)alerts.push({level:'pending',message:`${healthFreshness.state==='current'?'':'上次檢查：'}角色相容資料約 ${Math.ceil(capacity.maximumBytes/8000)}% 安全預算；請安排容量遷移，不會自動刪除資料。`});
...
let activePitrPreviewDatabaseId='',syncRecoveryRefreshTimer=null;
function scheduleSyncRecoveryCenterRefresh(delay=700){
 clearTimeout(syncRecoveryRefreshTimer);
 syncRecoveryRefreshTimer=setTimeout(()=>{syncRecoveryRefreshTimer=null;void renderSyncRecoveryCenter()},delay);
}
...
function schedulerSafeLesson(lesson={}){
 const allowed=['id','date','start','end','studentId','groupStudentIds','billingBranchId','teacherId','teacherIds','title','campId','room','location','branchId','deliveryMode','address','onlinePlatform','meetingUrl','status','note','seriesId','lessonState','isDraft'];
...
function schedulerSafeStudent(student={}){
 ...
 const allowed=['id','name','status','school','grade','level','preferredTeacherId','courseType','branchIds','isGroupRoster','groupMemberIds','billingBranchId','attendanceBranchId'];
...
async function renderCloudUserManager(){
 ...
 card.innerHTML=`<h2>老師帳號</h2><div class="small">帳號、本人老師與權限套裝分開儲存。一般老師只看本人課表；排課專員可管理全老師課表，但仍看不到費用、家長、薪資與公司資料。</div><label>老師</label><select id="cloudTeacherSelect"></select><label>老師 Gmail</label><input id="cloudTeacherEmail" type="email" placeholder="teacher@gmail.com"><label for="cloudTeacherPermissionPreset">權限套裝</label><select id="cloudTeacherPermissionPreset">${listAccessPresets('teacher').map(row=>`<option value="${row.id}">${row.label}</option>`).join('')}</select><br><button class="btn primary" id="saveCloudTeacherAccess">套用權限套裝</button><div id="cloudTeacherAccessList" class="backup-list" style="margin-top:12px"></div>`;
 ...
 document.getElementById('saveCloudTeacherAccess').onclick=async()=>{
   const button=document.getElementById('saveCloudTeacherAccess'),teacherId=sel.value,email=document.getElementById('cloudTeacherEmail').value.trim().toLowerCase(),presetId=document.getElementById('cloudTeacherPermissionPreset').value;
   if(!teacherId||!validGmailAddress(email))return alert('請選老師並輸入有效的 Gmail');
   const capabilities=buildAccessPreset(presetId,{teacherId}),canManageSchedule=isSchedulerAccess({...capabilities,active:true});
   ...
   try{
    ...
    const payload={email,...capabilities,companyId:COMPANY_ID,teacherName:teacherBadgeName(t),active:true,branchNames:deleteField(),managerName:deleteField(),scopedDb:deleteField(),scopedClientHash:deleteField(),scopedUpdatedAt:deleteField(),updatedAt:serverTimestamp()};
...
function roleAccessSignature(value={}){
 ...
 return JSON.stringify({role:String(value.role||''),teacherId:String(value.teacherId||''),branchIds,readOnly:value.readOnly===true,canSubmitOwnReports:value.canSubmitOwnReports!==false,canManageSchedule:value.canManageSchedule===true,canMoveSchedule:value.canMoveSchedule===true,hideFinancials:value.hideFinancials===true,canViewBranchFinance:value.canViewBranchFinance===true,scheduleBranchIds:[...(value.scheduleBranchIds||[])].map(String).sort()});
}
...
function applyRoleUI(profile,user){
 ...
 window.DanbridgeAccess?.setContext({role:cloudRole,branchIds:cloudBranchIds,teacherId:cloudTeacherId,email:cloudEmailKey,readOnly:profile.readOnly===true||cloudRole==='branch_manager',canSubmitOwnReports:profile.canSubmitOwnReports!==false,canManageSchedule:cloudCanManageSchedule,canMoveSchedule:cloudRole==='branch_manager'&&profile.canMoveSchedule===true,hideFinancials:cloudRole==='branch_manager'&&profile.hideFinancials===true,canViewBranchFinance:cloudRole==='branch_manager'&&profile.canViewBranchFinance===true,scheduleBranchIds:profile.scheduleBranchIds||[]});
 ...
 if(header){
   ...
   const label=document.createElement('span');label.className='cloud-role-label';label.style.cssText='font-size:12px;font-weight:800';label.textContent=`${cloudCanManageSchedule?'排課專員':(window.DanbridgeAccess?.ROLE_LABELS?.[profile.role]||profile.role)}｜${String(signedInName).trim()}`;header.append(label);
   if(profile.role==='owner'){
     const notice=document.createElement('button');notice.type='button';notice.className='btn notification-bell';notice.setAttribute('aria-label','開啟通知中心');
     const icon=document.createElement('span');icon.className='notification-bell-icon';icon.textContent='🔔';
     const count=document.createElement('span');count.id='notificationCount';count.className='notification-count';count.hidden=true;count.textContent='0';
     notice.append(icon,count);notice.addEventListener('click',()=>window.DanbridgeNotifications?.open?.());header.append(notice);
   }
   if(profile.role==='owner'&&DANBRIDGE_ENVIRONMENT==='production'){
     const privacy=document.createElement('button');privacy.type='button';privacy.className='btn';privacy.id='productionRolePrivacyBtn';privacy.textContent='驗證角色權限';header.append(privacy);
   }
   const logout=document.createElement('button');logout.type='button';logout.className='btn';logout.id='firebaseLogoutBtn';logout.textContent='登出';header.append(logout);
 }
 ...
 document.body.classList.toggle('branch-finance-hidden',cloudRole==='branch_manager'&&profile.hideFinancials===true&&profile.canViewBranchFinance!==true);
 ...
 if(teacherOnly){
   ...
   const teacherAllowedTabs=new Set(cloudCanManageSchedule?['calendar','teacherLeave','bookPurchase']:['dashboard','calendar','lessons','teacherLeave','bookPurchase']);
   const teacherTabLabels={bookPurchase:'書籍代購',dashboard:'我的總覽',calendar:cloudCanManageSchedule?'全老師課表':'我的課表',lessons:'課程回報',teacherLeave:cloudCanManageSchedule?'請假管理':'我的請假'};
   ...
   const branchFinanceVisible=profile.hideFinancials!==true||profile.canViewBranchFinance===true;
   ...
   const branchTabLabels={bookPurchase:'書籍代購',dashboard:'總覽',students:'學生／家長',teachers:'老師',calendar:'課表',lessons:'課程紀錄',makeups:'補課中心',settlement:'月底一鍵結算',finance:'公司財務'};
   ...
   const branchReadOnlyControls='#studentWorkspaceTabs > button,#studentRows button[onclick^="showStudentHistory("],#studentRows button[data-branch-readonly="history"],#finance .v181-finance-nav > button,#finance .scoped-payroll-retry,#settlement .scoped-payroll-retry';
...
function activeRoleRecordIdentity(access,email){
 if(isSchedulerAccess(access))return{email,kind:'scheduler',teacherId:String(access.teacherId),branchIds:[]};
...
function queueInitialActiveRoleRecordViews(){
 ...
 if(activeOwnerV2OperationSender){document.body.dataset.activeRoleRecordBootstrap=JSON.stringify({state:'server-managed',activationEpoch:activeRecordPageController?.diagnostics?.().activationEpoch||''});return}
...
async function publishScopedViews(sourceOverride=null,{recordAuthority=false,verifyReadback=false}={}){
 if(cloudRole!=='owner')return;
 try{
   ...
   if(DANBRIDGE_ENVIRONMENT==='production'){
     ...
     const sourceHash=recordDataHash(sourceDb),requestId=`roleview-${crypto.randomUUID()}`,response=await productionRoleViewPublishCall({schema:'danbridge-production-role-view-publish-v1',requestId,sourceHash,release:APP_RELEASE,...(verifyReadback?{verifyReadback:true}:{})}),data=response?.data;
   ...
   if(DANBRIDGE_ENVIRONMENT==='staging'&&activeRecordMode!=='legacy'){
     document.body.dataset.activeRoleRecordProgress='server-managed';
     return{state:'server-managed'};
   }
...
function announceScheduleDeliveryCompletion(){
 if(DANBRIDGE_ENVIRONMENT==='production'){
  const core=activeRecordPageController?.diagnostics?.(),counts=core?.counts||{};
  if(cloudRole!=='owner'||!core||!['complete','ready'].includes(core.state)||core.writeAllowed!==true||core.dirty||core.queued||core.inFlight||core.retryPending||['pending','sending','failed','quarantined'].some(key=>Number(counts[key])>0)||roleViewPublishInFlight||roleViewPublishQueued||roleViewRetryCount||scheduleNotificationDeliveryJobs.size)return false;
 }
 cloudStatus('課表與老師通知已同步。','ok');return true;
}
...
function buildScheduleNotificationRecipientGroups(accessRows,teacherChanges,lessonChanges,ownerEmail,ownerDisplayName){
 ...
 for(const row of accessRows||[]){
   ...
   if(isSchedulerAccess(a))schedulers.push({email,role:'scheduler',teacherName:a.teacherName||a.displayName||'排課專員',branchIds:[]});
...
async function publishScheduleChangeNotifications(previousDb,currentDb,batchKey,actor={}){
 ...
 const accessDocs=await getCompanyAccessDocs();
 const grouped=buildScheduleNotificationRecipientGroups(accessDocs.map(d=>({id:d.id,...(d.data()||{})})),teacherChanges,lessonChanges,OWNER_EMAIL,OWNER_DISPLAY_NAME);
...
function queueScheduleChangeNotifications(previousDb,currentDb,batchKey,actor={}){
 ...
 const deliver=async()=>{
   ...
   try{
    ...
    job.attempts=0;
    if(job.version===snapshot.version){scheduleNotificationDeliveryJobs.delete(key);document.body.dataset.scheduleNotificationDeliveryMs=String(Date.now()-job.queuedAt);announceScheduleDeliveryCompletion()}
...
function installScheduleNotificationUI(){
 ...
 document.body.appendChild(modal);
 document.getElementById('scheduleNotificationLater').onclick=()=>{scheduleNotificationPresenter?.deferCurrent();modal.hidden=true};
...
function renderScheduleNotification(notification){
 ...
 if(notification.notificationType==='book-purchase')body.innerHTML=`<p class="schedule-notification-lead"><b>書籍代購更新</b><span>${escapeHTML(notification.message||'書籍需求已更新')}</span></p><div class="schedule-notification-actions"><button class="btn" type="button" data-book-notification-open>查看書籍代購</button></div>`;
 else if(notification.notificationType==='teacher-leave')body.innerHTML=`<p class="schedule-notification-lead"><b>老師請假異動</b><span>${escapeHTML(notification.message||'請假紀錄已更新')}</span></p><div class="schedule-notification-table-wrap"><table class="schedule-notification-table"><thead><tr><th>老師</th><th>日期</th><th>時間</th><th>類別</th><th>時數</th><th>狀態</th></tr></thead><tbody>${details.map(item=>`<tr><td><b>${escapeHTML(item.teacherName||'老師')}</b></td><td>${escapeHTML(item.date||'')}</td><td>${escapeHTML(`${item.start||''}–${item.end||''}`)}</td><td>${escapeHTML(item.leaveTypeLabel||'')}</td><td>${escapeHTML(String(item.hours??''))}</td><td>${escapeHTML(teacherLeaveNotificationStatusLabel(item.status))}</td></tr>`).join('')}</tbody></table></div><div class="schedule-notification-actions"><button type="button" class="btn" data-leave-notification-open>查看請假紀錄</button></div><div class="schedule-notification-time">更新時間：${escapeHTML(formatNotificationTimestamp(notification.createdAt)||'剛剛')}</div>`;
 ...
 const notificationLead=body.querySelector('.schedule-notification-lead b');if(notificationLead&&!['teacher-leave','book-purchase'].includes(notification.notificationType))notificationLead.textContent=`${notification.createdByName||'Owner'} 已更新課表`;
 ...
 body.querySelector('[data-book-notification-open]')?.addEventListener('click',()=>{modal.hidden=true;window.switchTab?.('bookPurchase')});
...
async function acknowledgeCurrentScheduleNotification(){
 ...
 try{
   ...
   if(modal&&cloudUid===acknowledgingUid&&cloudEmailKey===acknowledgingEmail&&JSON.stringify(currentIds)===JSON.stringify(ids)){
    ...
    // User-initiated recovery keeps this page (and any unsaved work) intact.
    // The new page must obtain its own real App Check attestation. Never copy
    // tokens, change provider state, or automatically reload/replay the write.
    if(String(e?.code||'').startsWith('appCheck/')&&globalThis.location?.protocol==='https:'){
     const recovery=document.createElement('a');
     recovery.className='btn';recovery.href=globalThis.location.origin+'/';
     recovery.target='_blank';recovery.rel='noopener noreferrer';
     recovery.textContent='在新分頁重新驗證';
     const explanation=document.createElement('span');
     explanation.textContent=' 原分頁與未儲存內容會保留；新分頁驗證成功後，請再確認這則通知。 ';
     status.append(explanation,recovery);
    }
   }
 }
...
function subscribeScheduleNotifications(){
 ...
 scheduleNotificationPresenter=createScheduleNotificationPresenter({document,button:noticeButton,
  ...
  isBusy:()=>{const scheduler=productionSchedulerQueue?.diagnostics?.();return Boolean(document.querySelector('#lessonModal.show,.modal.show')||!document.getElementById('scheduleNotificationModal')?.hidden||activeRecordPageController?.diagnostics?.().inFlight||schedulerRequestWorkerActive||scheduler?.inFlight||scheduler?.pending||scheduler?.dirty)},
  render:sameType=>{
   const current=sameType[0],details=sameType.flatMap(n=>Array.isArray(n.details)?n.details:[]);
   renderScheduleNotification({...current,notificationIds:sameType.map(n=>n.id),details,message:['teacher-leave','book-purchase'].includes(current.notificationType)?current.message:(current.recipientRole==='owner'?`公司課表共有 ${details.length} 個變更`:current.recipientRole==='scheduler'?`全老師課表共有 ${details.length} 個變更`:current.recipientRole==='branch_manager'?`您管理的校區課表共有 ${details.length} 個變更`:`您的課表共有 ${details.length} 個變更`)});
  }
 });
 ...
 unsubscribeScheduleNotifications=onSnapshot(q,{includeMetadataChanges:true},snap=>{
   ...
   if(!snap.metadata.fromCache&&snap.docChanges().some(change=>change.type!=='removed'&&change.doc.data().notificationType==='book-purchase'&&scheduleNotificationMatchesFilters(change.doc.data(),notificationFilters)))window.dispatchEvent(new Event('danbridge:book-purchase-changed'));
   scheduleNotificationDocuments=snap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>scheduleNotificationMatchesFilters(x,notificationFilters)&&x.read!==true&&!scheduleNotificationExpired(x)).sort((a,b)=>{const priority=(b.notificationType==='teacher-leave')-(a.notificationType==='teacher-leave');if(priority)return priority;const at=a.createdAt?.toMillis?.()||0,bt=b.createdAt?.toMillis?.()||0;return bt-at});
...
async function auditProductionRoleViews(sourceDb){
 ...
 for(const accessDoc of accessDocs){
  ...
  if(access.role==='teacher'&&access.teacherId){
   if(isSchedulerAccess(access)){kind='scheduler';counts.schedulers++;expectedDb=filteredSchedulerDB(sourceDb)}
  ...
  storedDb=await readRoleViewForAudit({head,identity:{email,kind,teacherId:access.teacherId,branchIds:kind==='branch_manager'?access.branchIds:[]},expectedSourceHash:recordDataHash(sourceDb),readPart:async(id,scope)=>{const part=await getDocFromServer(doc(cloud,'productionRoleChunkViews',scope,'parts',id));if(!part.exists())throw Error('Role audit part missing');return part.data()},readCurrentHead:async()=>{const current=await getDocFromServer(roleRef);return current.exists()?current.data():null}});
  storedHash=String(head?.[kind==='branch_manager'?'scopedClientHash':'clientHash']||'');
...
window.__danbridgeRepublishProductionRoleViews=async()=>{
 ...
 const publication=await publishScopedViews(sourceDb,{recordAuthority:true,verifyReadback:true});
 const audit=publication?.audit;
 if(!audit?.verified||audit.sourceHash!==sourceHash||audit.formalRecordWrites!==0)throw Error('受保護後端尚未通過角色分塊完整讀回核對');
...
async function buildCurrentRoleViewCandidates(sourceDb){
 ...
 for(const accessDoc of accessDocs){
  ...
  let kind='',db=null;
  if(access.role==='teacher'&&access.teacherId){if(isSchedulerAccess(access)){kind='scheduler';db=filteredSchedulerDB(sourceDb)}else{kind='teacher';db=filteredTeacherDB(sourceDb,access.teacherId)}}
...
function currentRoleCandidateKind(){if(cloudRole==='teacher'&&cloudCanManageSchedule)return'scheduler';if(cloudRole==='teacher'&&!cloudCanManageSchedule)return'teacher';if(cloudRole==='branch_manager')return'branch_manager';return''}
...
function ensureActiveOwnerPageController(activationEpoch){
 ...
 const deviceId=activeOwnerDeviceIdentity(),journalKey=`${DANBRIDGE_ENVIRONMENT}:${cloudEmailKey}:${deviceId}:${activationEpoch}`,journalStorage=createBrowserOperationJournalStorage({indexedDB:window.indexedDB,locks:navigator.locks,key:journalKey}),journal=createOperationJournal({storage:journalStorage}),operationAdapter=DANBRIDGE_ENVIRONMENT==='production'?(productionTrustedOperationClient||(()=>{throw new Error('production trusted operation client unavailable')})()):(activeOwnerV2OperationSender||(()=>{throw new Error('staging V2 authority sender unavailable; legacy fallback forbidden')})()),conflictAdapter=DANBRIDGE_ENVIRONMENT==='production'?createFirebaseProductionRecordConflictAdapter({runTransaction:activeFirestoreTransaction,serverTimestamp,actor:{uid:cloudUid,email:cloudEmailKey},role:'owner'}):activeOwnerV2OperationSender?{persist:async(conflicts,context)=>{if(!stagingV2ConflictBackupCall)throw new Error('staging V2 conflict backup client unavailable');const result=await stagingV2ConflictBackupCall({conflicts,...context}),data=result?.data;if(data?.schema!=='danbridge-active-record-v2-conflict-backup-result-v1'||data.environment!=='staging'||data.companyId!==COMPANY_ID||data.activationEpoch!==context.activationEpoch||data.baseHash!==context.baseHash||data.targetHash!==context.targetHash||!Number.isSafeInteger(data.partCount)||data.partCount<1)throw new Error('staging V2 conflict backup response invalid');return data}}:(()=>{throw new Error('staging V2 conflict backup unavailable; legacy fallback forbidden')})(),failureResume=createRecordSyncActiveFailureResume({environment:DANBRIDGE_ENVIRONMENT,role:cloudRole,recordId:new URLSearchParams(location.search).get('recordSyncActiveFailureResume'),storage:sessionStorage,onDiagnostic:setActiveRecordSyncFailureResumeDiagnostic});
 activeOwnerControllerEpoch=activationEpoch;
 activeOwnerResumedEpoch='';
 const draftSessionKey=`danbridge-owner-draft-session:${journalKey}`;
 let draftSessionId=String(sessionStorage.getItem(draftSessionKey)||'');
 if(!/^[a-zA-Z0-9_-]{8,128}$/.test(draftSessionId)){draftSessionId=crypto.randomUUID();sessionStorage.setItem(draftSessionKey,draftSessionId)}
 const draftScope=`${journalKey}:draft:${draftSessionId}`;
 const draftStore=createOwnerDraftStore({scope:draftScope,storage:createBrowserOperationJournalStorage({indexedDB:window.indexedDB,locks:navigator.locks,key:draftScope})});
 activeRecordPageController=createActiveRecordPageController({
  ...
  sendBatch:typeof operationAdapter.applyBatch==='function'&&!new URLSearchParams(location.search).get('recordSyncActiveFailureResume')?operations=>operationAdapter.applyBatch(operations):null,
  persistConflicts:(conflicts,context)=>conflictAdapter.persist(conflicts,context),
  // The controller immediately snapshots this value; copying here duplicates the
  ...
  }:activeOwnerV2OperationSender?async confirmedDb=>{const confirmedSnapshot=deepCopy(confirmedDb);lastPublishedOwnerDB=confirmedSnapshot;ownerBaselineReady=true;activeRoleBootstrapSourceDb=confirmedSnapshot;document.body.dataset.activeRoleRecordProgress='server-verified';document.body.dataset.scheduleNotificationDelivery='server-verified';}:async confirmedDb=>{
   const before=lastPublishedOwnerDB?deepCopy(lastPublishedOwnerDB):null;
   await Promise.all([
...
async function runProductionHighRiskMutation({reason,mutate,requirePreview=false,confirmPreview}={}){
 ...
 const documents=await activeRecordPageController.readConfirmedDocuments(),remote=rebuildFullRecordShadowDb(documents,{environment:'production'}),target=deepCopy(remote.db),mutationResult=await mutate(target);
 let restoreHistory=null;
 if(reason==='restore'){
  if(typeof window.prepareBackupRestore!=='function')throw new Error('安全還原模組尚未載入，未執行任何變更');
  const prepared=window.prepareBackupRestore(remote.db,target);Object.assign(target,prepared.db);restoreHistory=prepared.history;
 }
 ...
 if(requirePreview){preview=await productionTrustedOperationClient.previewBatch(batch);if(typeof confirmPreview==='function'&&await confirmPreview({...preview,plan,mutationResult,restoreHistory})!==true)return{state:'cancelled',operationCount:plan.operationCount,preview};batch={...batch,previewId:preview.previewId}}
 const receipt=await productionTrustedOperationClient.applyBatch(batch);
 activeRecordPageController.acceptCommittedBatch(batch,receipt);
 // Capture the authoritative pre-commit state before accepting the receipt:
 // acceptActiveOwnerSnapshot also advances lastPublishedOwnerDB.
 const before=deepCopy(remote.db);
 await acceptActiveOwnerSnapshot({activationEpoch:plan.activationEpoch,db:plan.db,hash:plan.targetHash,writeAllowed:true});
 queueScheduleChangeNotifications(before,plan.db,plan.targetHash);
 // The trusted atomic commit has finished. Derived delivery uses the same
 // retry queue as normal edits and must not turn a committed delete into an
 // apparent failure or keep the next timetable action locked.
 publishRoleViewsWithRetry(plan.db);
 lastPublishedOwnerDB=deepCopy(plan.db);ownerBaselineReady=true;
...
async function confirmStagingV2DurablePrewriteBackup(){
 const user=auth.currentUser,uid=user?.uid,email=typeof user?.email==='string'?user.email.trim().toLowerCase():'';
 const epoch=activeOwnerControllerEpoch,fence=activeOwnerV2Fence;
 const assertCurrent=()=>{if(!user||auth.currentUser!==user||uid!==cloudUid||email!==cloudEmailKey||DANBRIDGE_ENVIRONMENT!=='staging'||cloudRole!=='owner'||!fence||activeOwnerV2Fence!==fence||activeOwnerControllerEpoch!==epoch||fence.targetV2Epoch!==epoch||activeOwnerV2HeadState!=='hn')throw new Error('staging V2 server prewrite verifier requires the same active Owner and Hn authority')};
 assertCurrent();await verifyActiveStagingV2Owner(user);assertCurrent();
 document.body.dataset.activeRecordPrewriteBackup=JSON.stringify({state:'server-enforced',projectId:firebaseConfig.projectId,targetV2Epoch:activeOwnerControllerEpoch,fenceHash:activeOwnerV2Fence.fenceHash});
...
async function startOwnerStagingV2Runtime(isCurrent=()=>true){
 ...
 activeRecordMode='checking';document.body.dataset.activeRecordMode=activeRecordMode;
 const fenceSnapshot=await getDocFromServer(doc(cloud,'stagingRecordSyncV1PermanentFences',COMPANY_ID));assertCurrent();
 if(!fenceSnapshot.exists())throw new Error('staging V2 permanent fence missing; legacy fallback forbidden');
 const fence=assertStagingV2PermanentFence(normalizeStagingV2FirestoreValue(fenceSnapshot.data())),epoch=fence.targetV2Epoch;
 await auth.currentUser?.getIdTokenResult?.(true);assertCurrent();warmStagingV2LimitedUseToken()?.catch(()=>{});
 const headReference=doc(cloud,'stagingActiveRecordV2Heads',COMPANY_ID,'epochs',epoch),headSnapshot=await getDocFromServer(headReference);assertCurrent();
 if(!headSnapshot.exists())throw new Error('staging V2 runtime head missing');
 let readState=assertStagingV2RuntimeHead(normalizeStagingV2FirestoreValue(headSnapshot.data()),epoch);
 const loader=createExplicitStagingV2AuthorityReadLoader();
 const readDocuments=async()=>{
  assertCurrent();const latestHeadSnapshot=await getDocFromServer(headReference);assertCurrent();
  if(!latestHeadSnapshot.exists())throw new Error('staging V2 runtime head missing during authority read');
  const latestState=assertStagingV2RuntimeHead(normalizeStagingV2FirestoreValue(latestHeadSnapshot.data()),epoch);
  if(readState==='hn'&&latestState!=='hn')throw new Error('staging V2 authority head regressed from Hn to H0');
  let documents;
  if(latestState==='h0')documents=stagingV2H0GenesisBaselineDocuments(await readActiveRecordDocumentsFromServer());
  else documents=(await loader.load({activationEpoch:epoch})).documentsByCollection;
  assertCurrent();readState=latestState;activeOwnerV2HeadState=readState;return documents;
 };
 const documents=await readDocuments();assertCurrent();
 const rebuilt=rebuildFullRecordShadowDb(documents,{environment:'staging'});
 activeOwnerV2Fence=fence;activeOwnerV2OperationSender=stagingV2BrowserOperationSender();activeOwnerV2ReadDocuments=readDocuments;
 ...
 try{
  ...
  activeRecordStreamAdapter=createFirebaseProductionRecordStreamAdapter({
   ...
   onState:event=>{
    ...
    if(event.state==='loading'){
     if(!productionRuntimeReady&&event.waitingFor==='collections')advanceCloudBootstrap('data',`雲端資料已確認 ${event.loadedCollectionCount}／${event.totalCollectionCount} 組`);
     if(!productionRuntimeReady&&event.waitingFor==='record-convergence')advanceCloudBootstrap('data','資料已讀取，正在核對雲端版本與完整性');
...
function startOwnerActiveRecordRuntime(){
 if(cloudRole!=='owner')return false;if(DANBRIDGE_ENVIRONMENT==='production')return startOwnerProductionRecordRuntime();if(DANBRIDGE_ENVIRONMENT!=='staging')return false;
 const generation=++ownerBootstrapGeneration,user=auth.currentUser,uid=cloudUid,email=cloudEmailKey;
 clearTimeout(ownerBootstrapRetryTimer);ownerBootstrapRetryTimer=null;
 const isCurrent=()=>generation===ownerBootstrapGeneration&&auth.currentUser===user&&cloudUid===uid&&cloudEmailKey===email&&cloudRole==='owner';
 let attempt=0;
 const run=async()=>{
  if(!isCurrent())return;
  try{await startOwnerStagingV2Runtime(isCurrent);if(isCurrent())delete document.body.dataset.activeRecordError}
   ...
   failCloudBootstrap(new Error(detail),{readOnly:true});cloudStatus(detail,retryable?'pending':'error');
   if(retryable)ownerBootstrapRetryTimer=setTimeout(()=>{ownerBootstrapRetryTimer=null;void run()},delay);
  }
 };
 void run();return true;
}
...
function queueOwnerCloudSave(options={}){
 ...
 if(scheduleMutation)document.body.dataset.scheduleAction=options.scheduleAction;
 if(scheduleMutation&&['staging','production'].includes(DANBRIDGE_ENVIRONMENT)&&activeRecordMode!=='legacy'){
  if(applyingCloud)return;
  if(!localDirtyHash)ownerRecoveryBaseDB=lastPublishedOwnerDB||null;
  localMutationVersion++;
  localDirtyHash=`schedule-pending:${localMutationVersion}`;
  ownerUploadQueued=true;
  scheduleOwnerSyncRecoveryPersistence(localMutationVersion);
  activeRecordPageController?.queueLocalSave({changedCollections:['lessons','makeups','changes']});
  cloudStatus(navigator.onLine?'課表已立即更新；逐筆變更正在依序同步。':'課表變更已保存在本機；恢復網路後依序續傳。',navigator.onLine?'pending':'offline');
  return;
 }
 if(studentMutation&&['staging','production'].includes(DANBRIDGE_ENVIRONMENT)&&activeRecordMode!=='legacy'){
  if(applyingCloud)return;
  if(!localDirtyHash)ownerRecoveryBaseDB=lastPublishedOwnerDB||null;
  localMutationVersion++;
  localDirtyHash=`student-pending:${localMutationVersion}`;
  ownerUploadQueued=true;
  scheduleOwnerSyncRecoveryPersistence(localMutationVersion);
  activeRecordPageController?.queueLocalSave({changedCollections:['students']});
  cloudStatus(navigator.onLine?'學生已立即加入；資料正在背景同步。':'學生已加入並保存在本機；恢復網路後自動同步。',navigator.onLine?'pending':'offline');
  return;
 }
...
function stopProductionSchedulerQueue(){
 branchMoveGeneration++;clearTimeout(branchMoveRetryTimer);branchMoveRetryTimer=null;
 const branchQueue=branchMoveController;branchMoveController=null;branchMoveInit=null;Promise.resolve(branchQueue?.stop()).catch(()=>{});
 ...
 Promise.resolve(queue?.stop()).finally(()=>release?.()).catch(()=>{});
}
async function initializeProductionSchedulerQueue(raw,sourceRecordRevision){
 if(productionSchedulerQueueInit)return productionSchedulerQueueInit;
 if(!Number.isSafeInteger(sourceRecordRevision)||sourceRecordRevision<0)throw new Error('排課視圖尚未附上新版權威版本，未改用舊寫入通道');
 const generation=productionSchedulerGeneration,identity={uid:cloudUid,email:cloudEmailKey},local=filteredSchedulerDB(window.__danbridgeGetDB?.()||emptyDB()),incoming=filteredSchedulerDB(raw);
 const current=()=>productionSchedulerGeneration===generation&&cloudUid===identity.uid&&cloudEmailKey===identity.email;
 let lease=null,queue=null;
 productionSchedulerQueueInit=(async()=>{
  const tabKey=`danbridge-scheduler-tab:${identity.email}`;let tabId=sessionStorage.getItem(tabKey);if(!tabId){tabId=crypto.randomUUID();sessionStorage.setItem(tabKey,tabId)}
  const journalKey=`production:scheduler:${identity.email}:${tabId}`;
  ...
  queue=createProductionSchedulerQueue({
   ...
   onState:event=>{
    ...
    const text=['confirmed','complete'].includes(event.state)?'本次課表、老師視圖與通知已由後端一起確認。':event.state==='ready'?'課表已讀取雲端版本，可繼續排課。':event.state==='blocked'?'排課尚未確認；操作已保留：'+event.error:'排課操作已排入永久佇列，依序同步中…';
    cloudStatus(text,['confirmed','complete','ready'].includes(event.state)?'ok':event.state==='blocked'?'error':'pending');
   }
  });
  const result=await queue.start({baselineDb:incoming,sourceRecordRevision});
  if(!current())throw new Error('排課登入狀態已改變');
  productionSchedulerQueue=queue;productionSchedulerLease=lease;initializing=false;schedulerStartupRecoveryChecked=true;
  await queue.acceptSnapshot(incoming,sourceRecordRevision);if(result.pending)flushProductionSchedulerQueue().catch(()=>{});
  return queue;
 })().catch(async error=>{await queue?.stop();await lease?.();if(current()){productionSchedulerQueue=null;productionSchedulerQueueInit=null;productionSchedulerLease=null}throw error});
 return productionSchedulerQueueInit;
}
async function flushProductionSchedulerQueue(){
 if(!productionSchedulerQueue||schedulerRecoveryHold)throw new Error('排課永久佇列尚未就緒或仍在救援鎖定');
 const generation=productionSchedulerGeneration,queue=productionSchedulerQueue;
 try{const result=await queue.flush();if(generation===productionSchedulerGeneration){schedulerUploadRetryCount=0;clearTimeout(schedulerUploadRetryTimer)}return result}
 catch(error){
  const code=String(error?.code||'').replace(/^functions\//,''),retryable=['unavailable','deadline-exceeded','internal','unknown','aborted','cancelled'].includes(code);
  if(retryable&&generation===productionSchedulerGeneration){schedulerUploadRetryCount++;clearTimeout(schedulerUploadRetryTimer);schedulerUploadRetryTimer=setTimeout(()=>{if(generation===productionSchedulerGeneration)flushProductionSchedulerQueue().catch(()=>{})},Math.min(15000,500*2**Math.min(schedulerUploadRetryCount,5)))}
  throw error;
 }
}
async function queueSchedulerChanges(){
 if(!cloudCanManageSchedule||applyingCloud)return;
 if(DANBRIDGE_ENVIRONMENT==='production')return scheduleSchedulerChanges();
...
async function applyActiveSchedulerRequest(requestRef,data){
 ...
 if(target.changed){
  applyingCloud=true;try{window.__danbridgeSetDB(deepCopy(target.after));persistCurrentLocalView();if(typeof window.renderVisibleWorkspace==='function')window.renderVisibleWorkspace();else window.renderAll?.()}finally{applyingCloud=false}
 ...
 if(marked&&target.changed)queueScheduleChangeNotifications(target.before,target.after,`scheduler-${requestSnapshot.id}`,{uid:currentData.actorUid,name:'排課專員'});
}
async function applySchedulerRequest(requestRef,data){
 ...
 if(notificationBefore&&notificationAfter)queueScheduleChangeNotifications(notificationBefore,notificationAfter,`scheduler-${requestRef.id}`,{uid:data.actorUid,name:'排課專員'});
}
...
function installCloudSave(){
 ...
 window.saveDB=function(options={}){
   if(cloudRole==='branch_manager'&&branchMoveController&&options.scheduleAction==='lesson.move'){
    const result=originalSaveDB?.(options);
    const generation=branchMoveGeneration;branchMoveController.move(window.__danbridgeGetDB()).catch(error=>{if(generation!==branchMoveGeneration)return;cloudStatus('校區移動尚未確認，操作已保留：'+error.message,'error');scheduleBranchMoveRetry(error)});return result;
   }
...
function scheduleBranchMoveRetry(error){
 clearTimeout(branchMoveRetryTimer);const generation=branchMoveGeneration;
 branchMoveRetryTimer=null;const code=String(error?.code||'').replace(/^functions\//,'');
 if(!['unavailable','deadline-exceeded','aborted','resource-exhausted','internal','network-request-failed'].includes(code))return;
 branchMoveRetryTimer=setTimeout(()=>{if(generation===branchMoveGeneration&&branchMoveController)branchMoveController.flush().catch(next=>{if(generation===branchMoveGeneration)scheduleBranchMoveRetry(next)})},5000);
}
async function acceptBranchMoveSnapshot(raw,revision,ref,identity,isActive){
 const generation=branchMoveGeneration;
 if(!branchMoveInit){
  branchMoveInit=(async()=>{
   const {createBranchScheduleMoveController}=await import('./branch-schedule-move.js?v=20.26.367');
   ...
   if(!isActive()||generation!==branchMoveGeneration){await controller.stop();throw Error('校區登入已變更')}
   branchMoveController=controller;window.attachDragHandlers?.();warmProductionSchedulerLimitedUseToken();
   if(controller.diagnostics().pending||controller.diagnostics().dirty)controller.flush().catch(error=>{if(generation===branchMoveGeneration)scheduleBranchMoveRetry(error)});return controller;
  })().catch(error=>{if(generation===branchMoveGeneration)branchMoveInit=null;throw error});
 }
...
function createPublishedRoleConsumer(ref,identity,applyView){
 ...
 const consumer=createPublishedRoleViewConsumer({identity,isActive,readCurrentHead,
  ...
  apply:async(db,snapshot)=>{
   ...
   }else if(DANBRIDGE_ENVIRONMENT==='production'&&identity.kind==='branch_manager'&&window.DanbridgeAccess?.getContext?.().canMoveSchedule===true){await acceptBranchMoveSnapshot(db,snapshot.sourceRecordRevision,ref,identity,isActive)}
...
async function subscribeSchedulerTeacherLegacy(){
 ...
 unsubscribeState=onSnapshot(ref,{includeMetadataChanges:true},async snap=>{
   ...
   if(DANBRIDGE_ENVIRONMENT==='production'){
    ...
    productionSchedulerViewChain=productionSchedulerViewChain.catch(()=>{}).then(async()=>{if(generation!==productionSchedulerGeneration)return;const queue=await initializeProductionSchedulerQueue(raw,view.sourceRecordRevision);if(generation===productionSchedulerGeneration&&!schedulerRecoveryHold)await queue.acceptSnapshot(raw,view.sourceRecordRevision)}).catch(error=>{if(generation===productionSchedulerGeneration)cloudStatus('排課資料未完成驗證，已保留本機：'+(error?.message||error),'error')});return;
   }
...
function subscribeRoleAccessGuard(){
 ...
 unsubscribeAccessGuard=onSnapshot(accessRef,snap=>{
   ...
   else if(!snap.metadata.fromCache&&access.financePayrollRevision!==payrollRevision){payrollRevision=access.financePayrollRevision;window.dispatchEvent(new Event('danbridge:finance-source-changed'))}
...
// An accepted PWA update must not unload a pending write or its recovery journal.
// This is a fresh synchronous read, not a previously displayed green status.
window.__danbridgeCanReloadForUpdate=()=>{
 const branch=branchMoveController?.diagnostics();if((branchMoveInit&&!branchMoveController)||branch?.dirty||branch?.pending||branch?.inFlight||branch?.error)return false;
 const owner=activeRecordPageController?.diagnostics?.(),scheduler=productionSchedulerQueue?.diagnostics?.();
 const counts=owner?.counts||{};
 return !Boolean(localDirtyHash||ownerUploadInFlight||ownerUploadQueued||roleViewPublishInFlight||roleViewPublishQueued||scheduleNotificationDeliveryJobs.size||schedulerRequestQueue.length||schedulerRequestWorkerActive||schedulerQuarantinedRequestIds.size||(productionSchedulerQueueInit&&!productionSchedulerQueue)||owner?.dirty||owner?.queued||owner?.inFlight||owner?.retryPending||counts.pending||counts.sending||counts.failed||counts.quarantined||scheduler?.pending||scheduler?.dirty||scheduler?.inFlight||scheduler?.error||window.__danbridgeLocalSnapshotState?.state==='saving');
};
window.addEventListener('offline',()=>setOfflineStatus());
function verifyForegroundRole(){if(document.visibilityState==='visible'&&navigator.onLine)activePublishedRoleConsumer?.refresh()}
window.addEventListener('online',()=>{cloudStatus('網路已恢復，正在檢查待同步變更…','pending');if(cloudRole==='owner'){ownerUploadQueued=true;clearTimeout(ownerRetryTimer);uploadOwnerState();scheduleDailyCloudBackup();renderSyncRecoveryCenter()}else{cloudStatus('網路已恢復，正在重新連線…','pending');verifyForegroundRole()}});
document.addEventListener('visibilitychange',()=>{if(document.visibilityState!=='visible')return;if(cloudRole==='owner'&&ownerUploadQueued&&navigator.onLine)uploadOwnerState();verifyForegroundRole()});
...
onAuthStateChanged(auth,async user=>{
 ...
 try{
   beginCloudBootstrap();cloudStatus('1/5 已確認 Google 登入','pending');const profile=await loadSignedInProfile(user);if(auth.currentUser?.uid!==user.uid)return;advanceCloudBootstrap('profile','帳號權限已從雲端讀回');cloudStatus('2/5 權限資料已確認','pending');applyRoleUI(profile,user);advanceCloudBootstrap('role',`${profile.role} 資料範圍已鎖定`);cloudStatus('3/5 角色範圍已確認','pending');showCloudApp();advanceCloudBootstrap('data','正在驗證角色專屬雲端資料');cloudStatus('4/5 正在驗證雲端資料','pending');
   if(profile.role==='owner'){subscribeOwner();if(!['staging','production'].includes(DANBRIDGE_ENVIRONMENT))subscribeSchedulerRequests();setTimeout(()=>{renderCloudUserManager();renderBranchManagerAccess()},0)}else if(profile.role==='teacher'&&profile.canManageSchedule===true)subscribeSchedulerTeacher();else if(profile.role==='teacher')subscribeTeacher();else if(profile.role==='branch_manager')subscribeBranchManager();else throw new Error('不支援的角色：'+profile.role);subscribeRoleAccessGuard();subscribeLessonReports();subscribeScheduleNotifications();subscribeTeacherLeaves();window.__danbridgeSetBookIdentity?.({uid:user.uid,email:String(user.email||'').toLowerCase(),name:profile.teacherName||profile.displayName||user.displayName||user.email});
   // Login metadata is not an authorization or data-readiness prerequisite.
   // Firestore may keep this write pending offline; never hold the verified
   // role's data subscriptions behind a best-effort timestamp update.
   void recordSuccessfulLogin(user,profile).catch(e=>{if(auth.currentUser?.uid!==user.uid)return;console.warn('最後登入時間更新失敗：',e);reportOperationalError(e,{category:'cloud-write',area:'access-guard',retryable:true})});
