...
export function buildStagingExecutionManifest({plan,sourceDb,sourceRevisions,targetDb,backup,restoreReceipt,legacyVersionHash,readBudget,writeBudget,createdAt}={}){
 ...
 const sourceRecordHash=recordDataHash(sourceDb),targetRecordHash=recordDataHash(targetDb),sourceSha256=sha256Canonical(sourceDb);
 ...
 try{verifyImmutableMigrationBackupManifest(backup,{currentSourceHash:sourceSha256})}catch{throw new Error('staging 執行前 verified 備份不符')}
 if(backup.schema!==IMMUTABLE_MIGRATION_BACKUP_SCHEMA||!id(backup.backupId)||backup.recordCount!==sourceRecordCount||!same(Object.fromEntries(FULL_RECORD_COLLECTIONS.map(collection=>[collection,backup.collections?.[collection]?.count])),plan.sourceCounts))throw new Error('staging 執行前 verified 備份不符');
 if(!restoreReceipt?.persisted||!id(restoreReceipt.drillId)||restoreReceipt.sourceBackupId!==backup.backupId||restoreReceipt.sourceHash!==sourceSha256||restoreReceipt.restoredHash!==sourceSha256||restoreReceipt.recordCount!==sourceRecordCount||restoreReceipt.mainUnchanged!==true)throw new Error('staging 執行前復原演練 receipt 不符');
