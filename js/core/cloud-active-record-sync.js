...
export function prepareActiveRecordSync({documentsByCollection,baselineDb,localDb,environment,deviceId,activationEpoch,startSequence=1}={}){
 if(!allowedEnvironment.has(environment)||!validText(deviceId)||!validText(activationEpoch)||!Number.isSafeInteger(startSequence)||startSequence<1)throw new Error('日常逐筆同步設定無效');
 ...
 const operations=raw.operations.map(row=>{
  ...
  return{schema:'danbridge-active-record-operation-v1',environment,companyId:'danbridge',activationEpoch,operationId,deviceId,collection,recordId,type:row.type,baseRevision:row.payload.revision-1,nextRevision:row.payload.revision,payload:clone(row.payload),path:row.path};
 });
