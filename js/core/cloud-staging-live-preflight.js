import {buildLiveOperationPlan} from './cloud-live-operation-plan.js';
...
export function buildStagingLivePreflight({environment,role,projectId,sourceState,targetDb,backup,restoreReceipt,legacyVersionHash,deviceId,startSequence=1,readBudget,writeBudget,createdAt}={}){
 if(environment!=='staging'||role!=='owner'||projectId!=='danbridge-d8877-staging')throw new Error('live 逐筆預檢只允許 staging Owner');
 const plan=buildLiveOperationPlan(sourceState,targetDb,{deviceId,startSequence,expectedBaseHash:recordDataHash(sourceState?.db)});
 const manifest=buildStagingExecutionManifest({plan,sourceDb:sourceState.db,sourceRevisions:sourceState.revisions,targetDb,backup,restoreReceipt,legacyVersionHash,readBudget,writeBudget,createdAt});
