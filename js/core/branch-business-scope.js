...
(function(){
  ...
  function scopedPeople(scope,lessons){
    const studentIds=new Set(lessons.flatMap(l=>lessonBillingStudentIds(l)));
  ...
  function branchBreakdown(lessons,teacherId){
    ...
    rows.filter(l=>!teacherId||lessonTeacherIds(l).includes(teacherId)).forEach(l=>{
      const id=teacherId?branchId(l):timetableBillingBranchId(l),row=grouped.get(id)||{branchId:id,h:0,amount:0,count:0};
  ...
  window.renderDashboard=function(){
    ...
    $('todayLessons').innerHTML=today.sort((a,b)=>a.start.localeCompare(b.start)).map(l=>`<div class="lesson ${hasTeacherOverlap(l)?'teacher-overlap':''}" style="--teacher:${safeCssColor(teacher(l.teacherId).color)};--location-bg:${locationBg(l.location)}" onclick="editLesson('${l.id}')"><b>${l.start}–${l.end}｜${esc(student(l.studentId).name)}</b><span class="meta">${esc(lessonTeacherNames(l))}｜${esc(l.title)}｜${esc(locationLabel(l))}｜${esc(l.room||'未指定教室')}</span></div>`).join('')||'<span class="small">今天沒有課程。</span>';
  ...
  window.renderFinance=function(){
    ...
    const expenseMatch=x=>financeScopeMatch(x,scope,r=>r?.branchId||'unassigned');
    $('fixedExpenseRows').innerHTML=(db.fixedExpenses||[]).filter(x=>fixedExpenseApplies(x,m)&&expenseMatch(x)).map(x=>`<tr><td><b>${esc(x.name)}</b><div class="small">${esc(scopeLabel(x.branchId||'unassigned'))}</div></td><td>${money(x.amount)}</td><td>${monthLabel(x.startMonth||'2026-07')}</td><td>${x.endMonth?monthLabel(x.endMonth):'持續'}</td><td>${expenseActions('editFixedExpense','deleteFixedExpense',x.id)}</td></tr>`).join('')||'<tr><td colspan="5" class="small">本月沒有生效中的固定開銷。</td></tr>';
    $('oneTimeExpenseRows').innerHTML=(db.oneTimeExpenses||[]).filter(x=>x.month===m&&expenseMatch(x)).slice().sort((a,b)=>b.month.localeCompare(a.month)).map(x=>`<tr><td>${monthLabel(x.month)}</td><td><b>${esc(x.name)}</b><div class="small">${esc(scopeLabel(x.branchId||'unassigned'))}</div></td><td>${money(x.amount)}</td><td>${expenseActions('editOneTimeExpense','deleteOneTimeExpense',x.id)}</td></tr>`).join('')||'<tr><td colspan="4" class="small">本月沒有一次性支出。</td></tr>';
