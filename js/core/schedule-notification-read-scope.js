...
export function scheduleNotificationReadFilters({email,role,teacherId='',branchIds=[],canManageSchedule=false,hideFinancials=false}={}){
 ...
 if(role==='branch_manager'&&Array.isArray(branchIds)&&branchIds.length&&branchIds.every(id=>typeof id==='string'&&id)&&new Set(branchIds).size===branchIds.length){
  ...
  return [...filters,['recipientRole','==','branch_manager'],['branchIds','==',[...branchIds]],...(hideFinancials===true?[['privacyScope','==','schedule-only-v1']]:[])];
 }
