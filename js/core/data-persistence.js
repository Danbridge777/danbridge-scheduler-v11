...
function saveDB(options={}){
 ...
 try{if(!lightweightMutation)window.DanbridgeLessonIndex?.invalidate?.()}catch(error){console.error('Lesson index invalidation failed:',error)}
 try{
...
const DANBRIDGE_BACKUP_RECEIPT_KEY='danbridge_verified_backup_receipt_v1';
...
const resetDataWithoutVerifiedBackup=resetData;
resetData=async function(){
 let receipt=null;try{receipt=JSON.parse(localStorage.getItem(DANBRIDGE_BACKUP_RECEIPT_KEY)||'null')}catch{}
 const receiptAge=receipt?.exportedAt?Date.now()-new Date(receipt.exportedAt).getTime():Infinity;
 if(!receipt||receipt.checksum!==backupChecksum(db)||receiptAge>86400000){alert('尚未清空：必須先匯出目前這一版的完整 JSON，且匯出時間需在 24 小時內。\n\n請先按「匯出最新資料」，確認檔案已保存後再重試。');return}
 return resetDataWithoutVerifiedBackup();
};
