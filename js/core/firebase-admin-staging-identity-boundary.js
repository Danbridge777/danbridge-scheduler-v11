...
const DATA_PERMISSIONS=Object.freeze(['datastore.databases.get','datastore.entities.create','datastore.entities.get','datastore.entities.list','datastore.entities.update','firebaserules.releases.create','firebaserules.releases.get','firebaserules.releases.update','firebaserules.rulesets.create','firebaserules.rulesets.get']);
...
export function verifyFirebaseAdminStagingIamReceiptShape(raw){
 ...
 const reviewedMs=timestampMs(row.reviewedAt),dataMember=`serviceAccount:${row.serviceAccountEmail}`,bindings=projectIam.bindings,saBindings=serviceAccountIam.bindings,expiries=[...bindings,...saBindings].map(item=>/timestamp\('([^']+)'\)/.exec(item.condition?.expression||'')?.[1]),expiry=expiries[0],expiryMs=timestampMs(expiry),projectExpression=`request.time < timestamp('${expiry}')`,tokenExpression=projectExpression,dataBinding=bindings.find(item=>item.role===roles.customData.fullName),usageBinding=bindings.find(item=>item.role===roles.serviceUsage.fullName),tokenBinding=saBindings[0];
