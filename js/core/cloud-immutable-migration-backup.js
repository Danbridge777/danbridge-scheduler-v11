import {createShardedSnapshot,assembleShardedSnapshot,SHARDED_DB_COLLECTION_KEYS} from './cloud-sharded-store.js?v=20.26.318';
...
export function canonicalJSONString(value){return JSON.stringify(stable(value))}
export function sha256Canonical(value){return sha256Text(canonicalJSONString(value)??'')}
export function sha256Text(text){
 if(typeof text!=='string')throw new Error('SHA-256 input must be text');
 const bytes=new TextEncoder().encode(text),length=bytes.length,bitLength=length*8,paddedLength=((length+9+63)>>6)<<6,buffer=new Uint8Array(paddedLength);buffer.set(bytes);buffer[length]=0x80;
