...
export async function runActiveRecordSync({journal,readDocuments,send,persistConflicts,baselineDb,localDb,environment,deviceId,activationEpoch,startSequence=1,maxOperations=1000,maxRebases=5,changedCollections=null,onProgress=()=>{}}={}){
 ...
 const prepare=async({replace=false,reason=''}={})=>{
  const documents=await readDocuments({force:replace,preferCache:!replace,reason}),plan=prepareActiveRecordSync({documentsByCollection:documents,baselineDb,localDb,environment,deviceId,activationEpoch,startSequence:sequence,changedCollections});sequence=plan.nextSequence;
