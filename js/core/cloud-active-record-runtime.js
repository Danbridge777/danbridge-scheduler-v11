...
export async function runActiveRecordSync({journal,readDocuments,send,persistConflicts,baselineDb,localDb,environment,deviceId,activationEpoch,startSequence=1,maxOperations=1000,maxRebases=5,changedCollections=null,appendOnlyChangesCount=0,trustedDocuments=null,verifiedRemote=null,onProgress=()=>{}}={}){
 ...
 const prepare=async({replace=false,reason=''}={})=>{
  ...
  const documents=trusted?trustedDocuments:await readDocuments({force:replace,preferCache:!replace,reason}),plan=prepareActiveRecordSync({documentsByCollection:documents,baselineDb,localDb,environment,deviceId,activationEpoch,startSequence:sequence,changedCollections,appendOnlyChangesCount:trusted?appendOnlyChangesCount:0,...(trusted?{authoritativeSourceHash:verifiedRemote.hash,verifiedRemote}:{})});sequence=plan.nextSequence;
 ...
 while(true){
  ...
  const worker=await runOperationWorker({journal,send,recoverInterrupted:false,maxOperations,maxBatchOperations:stagingBatch?120:8,maxBatchRecords:stagingBatch?90:8,maxBatchAudits:stagingBatch?30:8,onProgress});
