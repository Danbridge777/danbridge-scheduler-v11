 ...
 return Object.freeze({
  async handle(rawRequest){
   ...
   try{
    ...
    if(idClaims.aud!==cfg.expectedProjectId||idClaims.iss!==`https://securetoken.google.com/${cfg.expectedProjectId}`||!actor(uid)||idClaims.user_id!==uid||!email(emailValue)||idClaims.email_verified!==true||!Number.isSafeInteger(idClaims.auth_time)||idClaims.auth_time>now+30||now-idClaims.auth_time>900||!Number.isSafeInteger(idClaims.exp)||idClaims.exp<=now||save.actorUid!==uid||save.actorEmail!==emailValue)throw new GatewayError(403,'IDENTITY_BLOCKED');
    if(!plain(appClaims)||appResponse.alreadyConsumed!==false||appResponse.appId!==cfg.expectedAppId||appClaims.app_id!==cfg.expectedAppId||appClaims.sub!==cfg.expectedAppId||appClaims.iss!==`https://firebaseappcheck.googleapis.com/${STAGING_V2_AUTHORITY_SAVE_PROJECT_NUMBER}`||!Array.isArray(appClaims.aud)||Object.getPrototypeOf(appClaims.aud)!==Array.prototype||appClaims.aud.length!==2||appClaims.aud[0]!==STAGING_V2_AUTHORITY_SAVE_PROJECT_NUMBER||appClaims.aud[1]!==cfg.expectedProjectId||!Number.isSafeInteger(appClaims.iat)||appClaims.iat>now+30||!Number.isSafeInteger(appClaims.exp)||appClaims.exp<=now)throw new GatewayError(403,'APP_CHECK_BLOCKED');
