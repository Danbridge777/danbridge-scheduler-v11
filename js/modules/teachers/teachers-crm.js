...
function clearTeacherForm(){
  ['teacherId','teacherName','teacherDisplayName','teacherEmploymentStartDate','teacherRate','teacherBaseSalary','teacherOvertimeRate','teacherDeductionRate','teacherMinWeeklyHours','teacherSubjects','teacherNote'].forEach(id=>{const e=$(id);if(e)e.value=''});
  if($('teacherStandardDailyHours'))$('teacherStandardDailyHours').value='8';
...
function saveTeacher(){
  ...
  const standardDailyHours=Number($('teacherStandardDailyHours')?.value||8);
  ...
  if(!Number.isFinite(standardDailyHours)||standardDailyHours<1||standardDailyHours>12)return alert('標準每日工時必須介於 1 到 12 小時');
  const item={
    ...(old||{}),id,name,displayName,
    employmentStartDate:$('teacherEmploymentStartDate')?.value||'',
    standardDailyHours,
    payrollMode,
    rate:rate??0,
    baseSalary,
...
function editTeacher(id){
  ...
  if($('teacherEmploymentStartDate'))$('teacherEmploymentStartDate').value=t.employmentStartDate||'';
  if($('teacherStandardDailyHours'))$('teacherStandardDailyHours').value=t.standardDailyHours||8;
