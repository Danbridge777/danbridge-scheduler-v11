...
(function(){
  ...
  const branchId=r=>timetableBillingBranchId(r);
  ...
  window.renderTeacherKpi=function(){
    ...
    const scopedLessons=(db.lessons||[]).filter(l=>!l.isDraft&&l.date?.startsWith(month)&&(scope==='all'||branchId(l)===scope)&&(c.role!=='teacher'||lessonTeachers(l).includes(c.teacherId)));scopedLessons.forEach(l=>lessonBillingStudentIds(l).forEach(id=>students.add(id)));
    const totalRevenue=c.role==='teacher'?scopedLessons.reduce((sum,l)=>sum+timetableRevenueCharge(l),0):studentTuitionRevenue(month,scope)+summerCampRegistrationRevenue(month,scope);
