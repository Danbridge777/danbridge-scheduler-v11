...
(function(){
 ...
 function render(){
  ...
  $('bookPurchaseRows').innerHTML=visible.length?visible.map(row=>`<article class="book-purchase-item"><div class="book-purchase-item-head"><h3>${esc(row.title)}</h3><span class="book-purchase-quantity">${esc(row.quantity)} 本</span></div><p>${esc(row.publisher||'—')} · ISBN ${esc(row.isbn||'—')}</p><p class="book-purchase-note">${esc(row.note||'')}</p><div class="book-purchase-item-footer"><span>${esc(row.createdByName||row.createdByEmail)}</span>${outbox.some(e=>e.request.id===row.id)?'<span class="book-purchase-readonly">待同步</span>':''}${own(row)?`<div class="book-purchase-actions"><button class="btn" data-book-edit="${esc(row.id)}">編輯</button><button class="btn" data-book-delete="${esc(row.id)}">刪除</button></div>`:'<span class="book-purchase-readonly">僅供查看</span>'}</div></article>`).join(''):'<p class="book-purchase-empty">目前沒有符合的書籍需求</p>';
 }
