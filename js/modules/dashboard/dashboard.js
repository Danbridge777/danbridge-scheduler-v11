...
function renderDashboard(){const m=monthNow(),ls=db.lessons.filter(l=>!l.isDraft&&l.date.startsWith(m));$('mStudents').textContent=db.students.length;$('mTeachers').textContent=db.teachers.length;$('mLessons').textContent=ls.length;if($('mTeacherHours')){const completedTeacherLessons=ls.filter(l=>l.teacherReportStatus==='completed'||l.teacherReportStatus==='makeup_completed');$('mTeacherHours').textContent=`${completedTeacherLessons.reduce((sum,l)=>sum+hours(l.start,l.end),0).toFixed(1)} 小時`;const note=$('mTeacherHours').closest('.metric')?.querySelector('small');if(note)note.textContent=`${completedTeacherLessons.length} 堂已完成`;};$('mRevenue').textContent=money(studentTuitionRevenue(m)+summerCampRegistrationRevenue(m));$('mUnpaid').textContent=studentUnpaidTuitionLabel(m);$('mPayroll').textContent=money(ls.reduce((a,l)=>a+lessonPay(l),0));$('mMakeups').textContent=(db.makeups||[]).filter(x=>x.status==='pending').length;const tod=todayStr(),changes=(db.changes||[]).filter(c=>localDate(new Date(c.at))===tod);$('mChanges').textContent=changes.length;$('todayChanges').innerHTML=changes.length?`<table class="change-table"><thead><tr><th>時間</th><th>類型</th><th>學生／班級</th><th>原日期</th><th>原時間</th><th>新日期</th><th>新時間</th></tr></thead><tbody>${changes.slice(0,100).map(c=>`<tr><td>${new Date(c.at).toLocaleTimeString('zh-TW',{hour:'2-digit',minute:'2-digit'})}</td><td><b>${esc(c.type)}</b></td><td class="student-cell">${esc(student(c.studentId).name)}</td><td class="muted-cell">${esc(c.before?.date||'—')}</td><td>${esc(c.before?.start||'—')}</td><td class="muted-cell">${esc(c.after?.date||'—')}</td><td>${esc(c.after?.start||'—')}</td></tr>`).join('')}</tbody></table>`:'<div class="small" style="padding:12px">今天尚無課表異動。</div>'; $('todayLessons').innerHTML=db.lessons.filter(l=>!l.isDraft&&l.date===tod).sort((a,b)=>a.start.localeCompare(b.start)).map(l=>`<div class="lesson ${hasTeacherOverlap(l)?'teacher-overlap':''}" style="--teacher:${safeCssColor(teacher(l.teacherId).color)};--location-bg:${locationBg(l.location)}" onclick="editLesson('${l.id}')"><b>${l.start}–${l.end}｜${esc(student(l.studentId).name)}</b><span class="meta">${esc(lessonTeacherNames(l))}｜${esc(l.title)}｜📍${esc(locationLabel(l))}${l.location==='到府'&&l.address?' '+esc(l.address):''}｜${esc(l.room||'未指定教室')}｜${l.paymentStatus==='paid'?'已繳':'未繳'}</span></div>`).join('')||'<span class="small">今天沒有課程。</span>';window.renderTeacherLeaveDashboard?.()}
...
function enhanceDashboardV32(){
 ...
 const paymentBalance=billingCollectionBalance(month),unpaid=paymentBalance.items.filter(item=>item.amount>item.collected);
 if($('v32UnpaidCount'))$('v32UnpaidCount').textContent=`${unpaid.length} 筆費用待處理`;
