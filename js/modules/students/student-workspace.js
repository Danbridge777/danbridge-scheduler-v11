...
function ensureStudentWorkspace(){
  ...
  for(const [mode,id,label] of [['individual','individualStudentTab','家教'],['group','createGroupRosterButton','團班']]){
...
function setStudentWorkspaceView(mode){
  ...
  if(title)title.textContent=group?'團班登記':'學生／家長 CRM';if(nameLabel)nameLabel.textContent=group?'團班名稱 *':'學生姓名 *';if(listTitle)listTitle.textContent=group?'團班清單':'學生清單';
...
function switchStudentWorkspace(mode){
  ...
  const draft=studentWorkspaceDrafts.get(mode);setStudentWorkspaceView(mode);clearStudentForm();
...
function renderGroupFeePreview(){
  ...
  const total=document.createElement('p');total.className='small';total.textContent=`已選 ${rows.length} 位；鐘點費合計 ${money(hourly)}／小時。實際帳單以課表時數計算，按各學生與家長分開；安親仍只收一次月費。`;target.append(total);
}
...
document.addEventListener('DOMContentLoaded',()=>{ensureStudentWorkspace();setStudentWorkspaceView(studentWorkspaceMode);new MutationObserver(clearStudentWorkspaceDraftsOnIdentityChange).observe(document.body,{attributes:true,attributeFilter:['class','data-cloud-role']})});
