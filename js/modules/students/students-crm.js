...
function studentMatchesCrmFilters(s,q='',teacherId=''){if(s.scheduleReferenceOnly)return false;const text=[s.name,s.parent,s.contact,s.parentLine,s.parentEmail,s.school,s.grade,s.level,s.materials].join(' ').toLowerCase();return(!teacherId||studentTeacherIds(s).has(String(teacherId)))&&(!q||text.includes(q))}
...
const financialContext=window.DanbridgeAccess?.getContext?.()||{},hideFinancials=financialContext.hideFinancials===true&&financialContext.canViewBranchFinance!==true;
const lines=ls.slice(0,30).map(l=>`${l.date} ${l.start}–${l.end}｜${lessonTeacherNames(l)}｜${l.title||'課程'}｜${l.status}${hideFinancials?'':'｜'+(l.paymentStatus==='paid'?'已繳':'未繳')}`).join('\n');alert(`【${s.name||'學生'} CRM 歷程】\n學校／年級：${s.school||'—'} ${s.grade||''}\n程度：${s.level||'—'}\n教材：${s.materials||'—'}\n考試紀錄：${s.scores||'—'}\n\n最近課程（最多30筆）\n${lines||'尚無課程紀錄'}`)}
