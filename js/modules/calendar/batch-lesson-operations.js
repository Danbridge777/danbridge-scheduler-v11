...
function previewBatch(){
  ...
  $('batchPreview').innerHTML=`<p><span class="ok-text">可套用 ${ok} 堂</span>｜<span class="danger-text">衝突／無效 ${bad} 堂</span></p>`+rows.map(x=>`<div><span data-calendar-student-id="${esc(x.old.studentId)}">${esc(student(x.old.studentId).name)}</span>：${esc(x.old.date)} ${esc(x.old.start)} → ${esc(x.next.date)} ${esc(x.next.start)}｜${esc(locationLabel(x.next))}｜${esc(x.next.room||'未指定教室')} ${x.error?`<span class="danger-text">（${esc(x.error)}）</span>`:x.warning?`<span class="danger-text">（${esc(x.warning)}，允許套用）</span>`:''}</div>`).join('');
}
function applyBatch(){
