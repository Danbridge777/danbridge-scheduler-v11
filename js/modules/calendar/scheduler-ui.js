...
function renderCalendarAnalysis(){
  ...
  const gapWeeks=teacherGapWeekDays(gapRange).map(day=>{const rows=gapRows.filter(x=>x.date===day.date);return`<section class="gap-day${rows.length?'':' empty'}"><div class="gap-day-head"><b>${day.label}</b><span>${day.date.slice(5).replace('-','/')}</span><em>${rows.length} 個空堂</em></div><div class="gap-day-list">${rows.length?rows.map(x=>`<div class="gap-item"><b>${esc(x.teacher)}</b><span>${x.start}–${x.end}</span><span class="pill red">${fmtHours(x.gap/60)} hr</span></div>`).join(''):'<div class="gap-day-empty">沒有符合條件的空堂</div>'}</div></section>`}).join('');
