...
(function(){
 ...
 function outstandingPaymentGroups(lessons,today){
  const studentsById=new Map((db.students||[]).filter(s=>s?.id).map(s=>[String(s.id),s])),grouped=new Map(),eligible=lessons.filter(l=>!l.isDraft&&l.date<=today&&withinRetention(l.date,today,120)),byId=new Map(eligible.map(l=>[String(l.id),l])),months=[...new Set(eligible.map(l=>l.date.slice(0,7)))];
  for(const month of months){
   const balance=billingCollectionBalance(month),reviewIds=new Set(balance.reviewStudentIds.map(String));
   for(const charge of balance.items){
    const remaining=Math.max(0,charge.amount-charge.collected),studentId=String(charge.studentId);if(!remaining||!studentsById.has(studentId))continue;
    const identity=JSON.parse(charge.key),matching=identity[0]==='lesson'?[byId.get(String(identity[1]))].filter(Boolean):identity[0]==='monthly'?eligible.filter(l=>l.date.startsWith(month)&&lessonIncludesStudent(l,studentId)):[];
    if(!matching.length)continue;
    const key=`${month}|${studentId}`,row=grouped.get(key)||{studentId,student:studentsById.get(studentId),month,lessons:[],amount:0,requiresReview:false};
    row.lessons.push(...matching);row.amount+=remaining;row.requiresReview ||= reviewIds.has(studentId);grouped.set(key,row);
   }
  }
  for(const row of grouped.values())row.lessons=[...new Map(row.lessons.map(l=>[l.id,l])).values()];
 ...
 function buildNotifications(){
  ...
  if(c.role!=='teacher'){
   if(!(c.role==='branch_manager'&&c.hideFinancials===true&&c.canViewBranchFinance!==true))outstandingPaymentGroups(lessons,today).slice(0,80).forEach(row=>{const first=row.lessons[0],last=row.lessons[row.lessons.length-1],studentName=String(row.student?.name||'').trim(),parentName=String(row.student?.parent||'').trim();items.push(item({id:`unpaid:${row.studentId}:${last.date}:${row.lessons.length}`,category:'payment',severity:'warn',icon:'收',title:`${studentName}${parentName?`（家長：${parentName}）`:''} ${row.month} 費用${row.requiresReview?'需核對':'未收款'}`,detail:`${first.date}${last.date!==first.date?'～'+last.date:''}｜${row.lessons.length} 堂｜${row.requiresReview?'金額需核對':`未收 ${money(row.amount)}`}｜${locationLabel(last)}`,sort:`2-${first.date}-${first.start}`,action:{type:'collections',studentId:row.studentId,month:row.month}}))});
  ...
  const authorizedItems=c.role==='branch_manager'&&c.hideFinancials===true&&c.canViewBranchFinance!==true?items.filter(x=>x.category!=='payment'&&x.action?.type!=='settlement'&&x.action?.type!=='collections'):items;
  const dedup=[...new Map(authorizedItems.map(x=>[x.id,x])).values()];dedup.sort((a,b)=>a.sort.localeCompare(b.sort));return dedup
 }
 ...
 function render(){if(document.body.classList.contains('auth-locked')){state.items=[];close();const list=$id('notificationList');if(list)list.replaceChildren();const summary=$id('notificationSummary');if(summary)summary.textContent='登入後顯示通知';return}state.items=buildNotifications();const reads=readIds(),unread=state.items.filter(x=>!reads.has(x.id)).length,badge=$id('notificationCount');if(badge){badge.textContent=unread>99?'99+':String(unread);badge.hidden=!unread}const summary=$id('notificationSummary');if(summary)summary.textContent=unread?`${unread} 則未讀，共 ${state.items.length} 則待辦`:`目前 ${state.items.length} 則通知，已全部讀取`;document.querySelectorAll('.notification-tab').forEach(b=>b.classList.toggle('active',b.dataset.filter===state.filter));const list=$id('notificationList');if(!list)return;const rows=filtered();list.innerHTML=rows.length?rows.map(n=>`<button type="button" class="notification-item ${reads.has(n.id)?'':'unread'}" onclick="DanbridgeNotifications.openItem('${esc(n.id)}')"><span class="notification-icon ${esc(n.severity)}">${esc(n.icon)}</span><span class="notification-copy"><b>${esc(n.title)}</b><span>${esc(n.detail)}</span><small>${categoryLabel(n.category)}</small></span><span class="notification-dot ${reads.has(n.id)?'read':''}"></span></button>`).join(''):`<div class="notification-empty"><strong>這個分類目前沒有通知</strong><span>新的課程、回報或營運事項出現時會顯示在這裡。</span></div>`;const footer=$id('notificationFooter');if(footer)footer.textContent=`最後更新：${new Date().toLocaleTimeString('zh-TW',{hour:'2-digit',minute:'2-digit'})}`}
 function setFilter(v){state.filter=v;render()}
 function open(){if(document.body.classList.contains('auth-locked'))return;document.body.classList.add('notification-center-open');render()}
 function close(){document.body.classList.remove('notification-center-open')}
 function markAllRead(){if(document.body.classList.contains('auth-locked'))return;const reads=readIds();state.items.forEach(x=>reads.add(x.id));saveRead(reads);render()}
 function openItem(id){if(document.body.classList.contains('auth-locked'))return;const n=state.items.find(x=>x.id===id);if(!n)return;const reads=readIds();reads.add(id);saveRead(reads);render();close();const a=n.action||{};if(a.type==='report'){window.openLessonReport?.(a.lessonId);return}if(a.type==='calendar'){const input=$id('calendarDate');if(input&&a.date)input.value=a.date;switchTab('calendar');setTimeout(()=>a.lessonId&&window.editLesson?.(a.lessonId),40);return}if(a.type==='collections'){switchTab('finance');window.setFinanceWorkspaceMonth?.(a.month,true);document.querySelector('.v181-finance-nav button[data-pane="collections"]')?.click();return}if(a.type==='lessons'){const month=$id('lessonMonth'),studentFilter=$id('filterStudent');if(month&&a.month)month.value=a.month;if(studentFilter&&a.studentId)studentFilter.value=a.studentId;switchTab('lessons');setTimeout(()=>{window.renderLessons?.();if(a.lessonId)window.editLesson?.(a.lessonId)},40);return}if(a.type==='makeups'){switchTab('makeups');return}if(a.type==='settlement'){switchTab('settlement')}}
