...
function finishScheduleHistory(token,ids=[]){if(!token||token.kind!=='schedule-patch-v1')throw new Error('課表復原紀錄無效');const affected=new Set([...token.before.keys(),...ids.map(String)]),patches=[],afterById=new Map();for(let index=0;index<(db.lessons||[]).length;index++){const lesson=db.lessons[index],id=String(lesson.id);if(affected.has(id))afterById.set(id,{value:lesson,index})}for(const id of affected){const before=token.before.get(id)||{value:null,index:-1},after=afterById.get(id)||{value:null,index:-1},afterValue=after.value?cloneHistoryValue(after.value):null;if(JSON.stringify(before.value)===JSON.stringify(afterValue))continue;patches.push({id,before:before.value,beforeIndex:before.index,after:afterValue,afterIndex:after.index})}const makeupsAfter=cloneHistoryValue(db.makeups||[]);if(!patches.length&&JSON.stringify(token.makeupsBefore)===JSON.stringify(makeupsAfter))return false;pushHistoryEntry({kind:'schedule-patch-v1',patches,makeupsBefore:token.makeupsBefore,makeupsAfter});return true}
...
renderWeek=function(date,f){
  ...
  const visibleLessons=db.lessons.filter(l=>weekDates.has(l.date)&&lessonMatchesCalendar(l,f));
