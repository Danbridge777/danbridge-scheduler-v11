...
function beginScheduleHistory(ids=[]){const wanted=new Set(ids.map(String)),before=new Map();if(wanted.size)for(let index=0;index<(db.lessons||[]).length;index++){const lesson=db.lessons[index];if(wanted.has(String(lesson.id)))before.set(String(lesson.id),{value:cloneHistoryValue(lesson),index})}return{kind:'schedule-patch-v1',before,makeupsBefore:cloneHistoryValue(db.makeups||[])}}
function finishScheduleHistory(token,ids=[]){if(!token||token.kind!=='schedule-patch-v1')throw new Error('課表復原紀錄無效');const affected=new Set([...token.before.keys(),...ids.map(String)]),patches=[],indexPatches=[],afterById=new Map();for(let index=0;index<(db.lessons||[]).length;index++){const lesson=db.lessons[index],id=String(lesson.id);if(affected.has(id))afterById.set(id,{value:lesson,index})}for(const id of affected){const before=token.before.get(id)||{value:null,index:-1},after=afterById.get(id)||{value:null,index:-1},afterValue=after.value?cloneHistoryValue(after.value):null;if(JSON.stringify(before.value)===JSON.stringify(afterValue))continue;patches.push({id,before:before.value,beforeIndex:before.index,after:afterValue,afterIndex:after.index});indexPatches.push({id,before:before.value,after:after.value||null})}const makeupsAfter=cloneHistoryValue(db.makeups||[]);if(!patches.length&&JSON.stringify(token.makeupsBefore)===JSON.stringify(makeupsAfter))return false;try{window.DanbridgeLessonIndex?.applyPatches?.(indexPatches,db.lessons)}catch(error){window.DanbridgeLessonIndex?.invalidate?.();console.error('Lesson index patch failed:',error)}pushHistoryEntry({kind:'schedule-patch-v1',patches,makeupsBefore:token.makeupsBefore,makeupsAfter});return true}
function toast(msg){const e=$('toast');e.textContent=msg;e.classList.add('show');setTimeout(()=>e.classList.remove('show'),1600)}
function updateUndoRedoButtons(){const u=$('undoBtn'),r=$('redoBtn');if(u)u.disabled=!historyStack.length;if(r)r.disabled=!redoStack.length}
function applyScheduleHistoryEntry(entry,direction,action){const currentLessons=(db.lessons||[]).map(row=>row),affected=new Set(entry.patches.map(row=>row.id)),next=currentLessons.filter(row=>!affected.has(String(row.id))),suffix=direction==='before'?'before':'after',indexKey=direction==='before'?'beforeIndex':'afterIndex',insertions=entry.patches.filter(row=>row[suffix]).map(row=>({value:cloneHistoryValue(row[suffix]),index:row[indexKey]})).sort((a,b)=>a.index-b.index);for(const row of insertions)next.splice(Math.max(0,Math.min(row.index,next.length)),0,row.value);const transitions=window.DanbridgePermanentOperationHistory.lessonTransitions(currentLessons,next);db.lessons=next;db.makeups=cloneHistoryValue(direction==='before'?entry.makeupsBefore:entry.makeupsAfter);for(const row of transitions){const kind=row.kind==='create'?'重建課程':row.kind==='delete'?'刪除課程':'修改課程';logChange(`${action}：${kind}`,row.after,row.before,{historyAction:action})}selectedLessonIds.clear();selectionMode=false;if(typeof commitScheduleMutation==='function')commitScheduleMutation('lesson.update.fields');else saveDB({scheduleAction:'lesson.update.fields'})}
function applyHistoryState(serialized,action){const current=JSON.parse(JSON.stringify(db)),target=JSON.parse(serialized),history=JSON.parse(JSON.stringify(current.changes||[])),transitions=window.DanbridgePermanentOperationHistory.lessonTransitions(current.lessons||[],target.lessons||[]);db=target;db.changes=history;for(const row of transitions){const kind=row.kind==='create'?'重建課程':row.kind==='delete'?'刪除課程':'修改課程';logChange(`${action}：${kind}`,row.after,row.before,{historyAction:action})}selectedLessonIds.clear();selectionMode=false;saveDB()}
function undoLast(){const x=historyStack.pop();if(!x)return toast('沒有可復原的操作');if(x?.kind==='schedule-patch-v1'){redoStack.push(x);if(redoStack.length>100)redoStack.shift();applyScheduleHistoryEntry(x,'before','全域復原')}else{redoStack.push(JSON.stringify(db));if(redoStack.length>100)redoStack.shift();applyHistoryState(x,'全域復原')}updateUndoRedoButtons();toast('已復原上一個操作')}
function redoLast(){const x=redoStack.pop();if(!x)return toast('沒有可重做的操作');if(x?.kind==='schedule-patch-v1'){historyStack.push(x);if(historyStack.length>100)historyStack.shift();applyScheduleHistoryEntry(x,'after','全域重做')}else{historyStack.push(JSON.stringify(db));if(historyStack.length>100)historyStack.shift();applyHistoryState(x,'全域重做')}updateUndoRedoButtons();toast('已重做上一個操作')}
function student(id){return db.students.find(x=>x.id===id)||{}}function teacher(id){return db.teachers.find(x=>x.id===id)||{}}function lessonTeacherIds(l){const ids=isGroupStudentId(l?.studentId)&&Array.isArray(l?.teacherIds)&&l.teacherIds.length?l.teacherIds:[l?.teacherId||(Array.isArray(l?.teacherIds)?l.teacherIds[0]:'')];return [...new Set(ids.filter(Boolean))]}function lessonTeacherNames(l){return lessonTeacherIds(l).map(id=>teacher(id).name).filter(Boolean).join('、')}function isGroupStudentId(id){return student(id).courseType==='團班'}function normalizeCampCode(v){return String(v||'').trim().replace(/\s+/g,'').toUpperCase()}function effectiveCampId(l){return normalizeCampCode(l?.campId)}function legacyCampAliases(l){return new Set([normalizeCampCode(l?.title),normalizeCampCode(student(l?.studentId).name)].filter(Boolean))}function sameCampIdentity(a,b){const ca=effectiveCampId(a),cb=effectiveCampId(b);if(ca&&cb)return ca===cb;if(ca&&!cb)return legacyCampAliases(b).has(ca);if(!ca&&cb)return legacyCampAliases(a).has(cb);return false}function sameCampSlot(a,b){
...
function syncCampField(){const wrap=$('lessonCampIdWrap');if(!wrap)return;const group=isGroupStudentId($('lessonStudent')?.value||'');wrap.classList.toggle('hidden',!group);if(!group&&$('lessonCampId'))$('lessonCampId').value=''}
function handleLessonStudentChange(){renderLessonGroupRoster();if(!$('lessonId')?.value&&typeof applyStudentScheduleDefaults==='function'&&applyStudentScheduleDefaults())return;if($('lessonDeliveryMode')?.value==='home'){const addr=student($('lessonStudent')?.value||'').homeAddress||'';if(addr&&!$('lessonAddress').value.trim())$('lessonAddress').value=addr}handleLocationChange();syncCoTeacherOptions();syncCampField()}
function branchRecord(id){return (db.branches||window.DanbridgeAccess?.DEFAULT_BRANCHES||[]).find(b=>b.id===id)}
function renderLessonBranchOptions(){const el=$('lessonBranch');if(!el)return;const current=el.value,branches=(db.branches||window.DanbridgeAccess?.DEFAULT_BRANCHES||[]).filter(b=>!['home_service','online','unassigned'].includes(b.id));el.innerHTML='<option value="">請選擇校區</option>'+branches.map(b=>`<option value="${esc(b.id)}">${esc(b.name)}</option>`).join('');if(current)el.value=current}
function handleBranchChange(){const branch=branchRecord($('lessonBranch')?.value||''),room=$('lessonRoom'),current=room?.value||'';if(room){const rooms=branch?.rooms||[];room.innerHTML='<option value="">未指定教室</option>'+rooms.map(r=>`<option value="${esc(r)}">${esc(r)}</option>`).join('')+(current&&!rooms.includes(current)?`<option value="${esc(current)}">${esc(current)}</option>`:'');room.value=current}$('lessonUnassignedWarning')?.classList.toggle('hidden',!!$('lessonBranch')?.value);handleLocationChange()}
function locationBg(loc){return loc==='河西一路'?'#dbeafe':loc==='到府'?'#ffedd5':loc==='線上課'?'#ede9fe':'#dcfce7'}
function locationLabel(l){const mode=window.DanbridgeAccess?.deliveryModeFromLesson?.(l)||'onsite',branch=window.DanbridgeAccess?.branchName?.(l.branchId)||l.location||'未歸屬校區';return mode==='home'?`${branch}・到府`:mode==='online'?`${branch}・線上`:branch}
function handleLocationChange(){renderLessonBranchOptions();const mode=$('lessonDeliveryMode')?.value||'onsite',branch=branchRecord($('lessonBranch')?.value||'');$('lessonLocation').value=mode==='home'?'到府':mode==='online'?'線上課':(branch?.name||'');$('lessonRoomWrap')?.classList.toggle('hidden',mode!=='onsite');$('lessonAddressWrap').classList.toggle('hidden',mode!=='home');$('lessonOnlineWrap')?.classList.toggle('hidden',mode!=='online');if(mode==='home'&&!$('lessonAddress').value)$('lessonAddress').value=student($('lessonStudent').value).homeAddress||'';if(mode!=='onsite')$('lessonRoom').value='';if(mode!=='home')$('lessonAddress').value='';if(mode!=='online'){if($('lessonMeetingUrl'))$('lessonMeetingUrl').value=''}renderLessonMapLink()}
function renderLessonMapLink(){const a=$('lessonAddress')?.value.trim()||'',box=$('lessonMapLinkWrap');if(!box)return;box.innerHTML=a?`<a class="map-link" href="${mapUrl(a)}" target="_blank" rel="noopener">📍 開啟 Google Maps</a>`:''}
function logChange(type,lesson,before=null,metadata={}){const clone=v=>v==null?v:JSON.parse(JSON.stringify(v)),ctx=window.DanbridgeAccess?.getContext?.()||{},name=(document.body.dataset.cloudDisplayName||ctx.email||'目前使用者').trim(),undoOfChangeId=String(metadata?.undoOfChangeId||'').trim(),historyAction=String(metadata?.historyAction||'').trim(),extra={...(undoOfChangeId?{undoOfChangeId:undoOfChangeId.slice(0,200)}:{}),...(historyAction?{historyAction:historyAction.slice(0,80)}:{})};db.changes||=[];db.changes.unshift({id:uid(),at:new Date().toISOString(),type,lessonId:lesson?.id||before?.id||'',studentId:lesson?.studentId||before?.studentId||'',actorName:name,actorEmail:ctx.email||'',before:clone(before),after:clone(lesson),...extra})}



function syncQuickStudentBillingFields(){const courseType=$('quickCourseType')?.value||'1對1',monthly=courseType==='安親',billing=$('quickBilling'),rate=$('quickRate');if(billing){billing.value=normalizedStudentBilling(courseType);billing.disabled=true}if(rate){const label=rate.previousElementSibling;if(label)label.textContent=monthly?'每月月費':'每小時鐘點費';rate.placeholder=monthly?'請輸入每月固定月費':'請輸入每小時鐘點費'}}
function toggleQuickStudent(show=true){const box=$('quickStudentBox');if(!box)return;box.classList.toggle('hidden',!show);if(show){syncQuickStudentBillingFields();$('quickStudentName').focus()}else{clearQuickStudentForm()}}function clearQuickStudentForm(){['quickStudentName','quickParentName','quickParentContact','quickHomeAddress','quickRate','quickBillingBranch'].forEach(id=>{if($(id))$(id).value=''});if($('quickCourseType'))$('quickCourseType').value='1對1';syncQuickStudentBillingFields();if($('v20QuickTeacher'))$('v20QuickTeacher').value='';if($('v20QuickBranch'))$('v20QuickBranch').selectedIndex=0;if($('v20QuickDuration'))$('v20QuickDuration').value='60'}function saveQuickStudent(){const role=window.currentCloudRole?.()||window.DanbridgeAccess?.getContext?.().role||'';if(role!=='owner')return alert('目前帳號沒有新增學生的權限。');const name=$('quickStudentName').value.trim();if(!name)return alert('請輸入學生姓名');const duplicate=db.students.find(s=>String(s.name||'').trim().toLowerCase()===name.toLowerCase());if(duplicate&&!confirm(`已有「${name}」，仍要新增嗎？`)){$('lessonStudent').value=duplicate.id;toggleQuickStudent(false);handleLessonStudentChange();return}const branch=$('v20QuickBranch')?.value||'',duration=+$('v20QuickDuration')?.value||60,courseType=$('quickCourseType').value;snapshot();const o=studentDefaults({id:uid(),name,parent:$('quickParentName').value.trim(),contact:$('quickParentContact').value.trim(),homeAddress:$('quickHomeAddress').value.trim(),courseType,billing:normalizedStudentBilling(courseType),rate:+$('quickRate').value||0,preferredTeacherId:$('v20QuickTeacher')?.value||'',branchIds:branch?[branch]:[],attendanceBranchId:branch,billingBranchId:$('quickBillingBranch')?.value||'',note:''});db.students.push(o);saveDB();renderSelects();$('lessonStudent').value=o.id;if(o.preferredTeacherId)$('lessonTeacher').value=o.preferredTeacherId;if(branch){$('lessonBranch').value=branch;handleBranchChange()}$('endTime').value=addMinutes($('startTime').value,duration);toggleQuickStudent(false);handleLessonStudentChange();window.realtimeConflicts?.();toast(`已新增並選取 ${o.name}`)}
...
function renderMonth(date,f){const y=date.getFullYear(),m=date.getMonth(),first=new Date(y,m,1),last=new Date(y,m+1,0),offset=(first.getDay()+6)%7,total=Math.ceil((offset+last.getDate())/7)*7;let h='<div class="month-grid">'+['一','二','三','四','五','六','日'].map(x=>`<div class="day-head">週${x}</div>`).join('');for(let i=0;i<total;i++){const d=new Date(y,m,1-offset+i),ds=localDate(d),out=d.getMonth()!==m,ls=window.DanbridgeLessonIndex.byDate(db.lessons,ds).filter(l=>lessonMatchesCalendar(l,f)).sort((a,b)=>a.start.localeCompare(b.start));h+=`<div class="day-cell ${out?'out':''} ${ds===todayStr()?'today':''}" data-date="${ds}" onclick="cellClick(event,'${ds}')"><div class="day-num"><span>${d.getDate()}</span><button type="button" class="day-add" aria-label="在 ${ds} 新增課程" title="新增課程" onclick="event.stopPropagation();openLessonModal('${ds}')">＋</button></div>${ls.map(lessonCard).join('')}</div>`}h+='</div>';$('calendarCanvas').innerHTML=h;$('calendarTitle').textContent=`${y} 年 ${m+1} 月`;attachDragHandlers()}function lessonCard(l){const t=teacher(l.teacherId),s=student(l.studentId),sel=selectedLessonIds.has(l.id);return `<div class="lesson ${(l.lessonState==='draft'||l.isDraft)?'lesson-draft':''} ${sel?'selected':''} ${hasTeacherOverlap(l)?'teacher-overlap':''} ${calendarFilterState().search?'calendar-search-hit':''}" title="${esc(lessonHoverText(l))}" draggable="${selectionMode?'false':'true'}" data-id="${l.id}" style="--teacher:${safeCssColor(t.color)};--location-bg:${locationBg(l.location)}"><b>${l.isDraft?'<span class=\"draft-tag\">草稿</span> ':''}${l.start}–${l.end}｜${esc(s.name)}</b><span class="meta">${esc(lessonTeacherNames(l))}${effectiveCampId(l)?`｜營隊:${esc(effectiveCampId(l))}`:''}｜📍${esc(locationLabel(l))}${l.location==='到府'&&l.address?' '+esc(l.address):''}｜${esc(l.title||'')}｜${esc(l.room||'')}｜${l.paymentStatus==='paid'?'✓已繳':'未繳'}${hasTeacherOverlap(l)?`｜⚠老師重複：${esc(lessonTeacherConflictNames(l).join('、'))}`:''}</span>${l.teacherReportStatus?`<div class="lesson-report-summary"><span class="report-badge ${esc(l.teacherReportStatus)}">${esc(window.teacherReportLabel?.(l.teacherReportStatus)||l.teacherReportStatus)}</span>${l.teacherReportContent?`<br>內容：${esc(l.teacherReportContent)}`:''}${l.teacherReportHomework?`<br>作業：${esc(l.teacherReportHomework)}`:''}</div>`:''}</div>`}function renderMobileWeek(date,f){const day=date.getDay(),mon=new Date(date);mon.setDate(date.getDate()-((day+6)%7));const days=Array.from({length:7},(_,i)=>{const d=new Date(mon);d.setDate(mon.getDate()+i);return d}),ctx=window.DanbridgeAccess?.getContext?.()||{},role=window.currentCloudRole?.()||ctx.role||'',canAdd=role==='owner'||ctx.canManageSchedule===true;let h='<div class="mobile-week-agenda">';for(const d of days){const ds=localDate(d),ls=window.DanbridgeLessonIndex.byDate(db.lessons,ds).filter(l=>lessonMatchesCalendar(l,f)).sort((a,b)=>a.start.localeCompare(b.start));h+=`<section class="mobile-week-day ${ds===todayStr()?'today':''}"><header><div><b>${d.getMonth()+1}月${d.getDate()}日・星期${weekday(ds)}</b><span>${ls.length} 堂課</span></div>${canAdd?`<button type="button" class="btn mobile-week-add" onclick="openLessonModal('${ds}')">＋</button>`:''}</header><div class="mobile-week-lessons">${ls.length?ls.map(lessonCard).join(''):'<p>沒有課程</p>'}</div></section>`}h+='</div>';$('calendarCanvas').innerHTML=h;$('calendarTitle').textContent=`${localDate(days[0])} ～ ${localDate(days[6])}`;attachDragHandlers()}function renderWeek(date,f){if(matchMedia('(max-width:700px)').matches)return renderMobileWeek(date,f);const day=date.getDay(),mon=new Date(date);mon.setDate(date.getDate()-((day+6)%7));const days=Array.from({length:7},(_,i)=>{const d=new Date(mon);d.setDate(mon.getDate()+i);return d}),startHour=8,endHour=22,totalSlots=(endHour-startHour)*12;let h='<div class="week-grid"><div class="week-head" style="grid-column:1;grid-row:1">時間</div>'+days.map((d,i)=>`<div class="week-head" style="grid-column:${i+2};grid-row:1">${d.getMonth()+1}/${d.getDate()} 週${weekday(localDate(d))}</div>`).join('');for(let slot=0;slot<totalSlots;slot++){const totalMin=startHour*60+slot*5,hr=Math.floor(totalMin/60),min=totalMin%60,time=`${String(hr).padStart(2,'0')}:${String(min).padStart(2,'0')}`,row=slot+2,isHour=min===0,isHalf=min===30,label=time;h+=`<div class="time-label ${isHour?'hour':isHalf?'half-hour':''}" style="grid-column:1;grid-row:${row}">${label}</div>`;for(let i=0;i<days.length;i++){const ds=localDate(days[i]),lineClass=isHour?'full-hour':isHalf?'half-hour':'';h+=`<div class="time-slot ${lineClass}" style="grid-column:${i+2};grid-row:${row}" data-date="${ds}" data-time="${time}" onclick="weekCellClick(event,'${ds}','${time}')"></div>`}}for(let i=0;i<days.length;i++){const ds=localDate(days[i]),ls=window.DanbridgeLessonIndex.byDate(db.lessons,ds).filter(l=>lessonMatchesCalendar(l,f)).sort((a,b)=>a.start.localeCompare(b.start));for(const l of ls){const [sh,sm]=l.start.split(':').map(Number),[eh,em]=l.end.split(':').map(Number),startMin=sh*60+sm,endMin=eh*60+em,clampedStart=Math.max(startHour*60,Math.min(endHour*60-5,startMin)),clampedEnd=Math.max(clampedStart+5,Math.min(endHour*60,endMin)),startSlot=Math.round((clampedStart-startHour*60)/5),span=Math.max(1,Math.round((clampedEnd-clampedStart)/5)),rowStart=startSlot+2,rowEnd=Math.min(totalSlots+2,rowStart+span);h+=`<div class="week-event ${(l.lessonState==='draft'||l.isDraft)?'lesson-draft':''} ${selectedLessonIds.has(l.id)?'selected':''} ${hasTeacherOverlap(l)?'teacher-overlap':''} ${calendarFilterState().search?'calendar-search-hit':''}" title="${esc(lessonHoverText(l))}" draggable="${selectionMode?'false':'true'}" data-id="${l.id}" style="grid-column:${i+2};grid-row:${rowStart}/${rowEnd};--teacher:${safeCssColor(teacher(l.teacherId).color)};--location-bg:${locationBg(l.location)}"><b>${l.isDraft?'<span class=\"draft-tag\">草稿</span> ':''}${l.start}–${l.end} ${esc(student(l.studentId).name)}</b><span class="week-event-meta">${esc(lessonTeacherNames(l))}${effectiveCampId(l)?`｜營隊:${esc(effectiveCampId(l))}`:''}｜📍${esc(locationLabel(l))}${hasTeacherOverlap(l)?`｜⚠${esc(lessonTeacherConflictNames(l).join('、'))}`:''}</span></div>`}}h+='</div>';$('calendarCanvas').innerHTML=h;$('calendarTitle').textContent=`${localDate(days[0])} ～ ${localDate(days[6])}｜每 5 分鐘一格`;attachDragHandlers()}
...
lessonCard=function(l){
  ...
  const teacherRole=calendarIsTeacherView()||(window.currentCloudRole?.()||window.DanbridgeAccess?.getContext?.().role||'')==='teacher';
  return teacherRole?html.replace(/｜(?:✓已繳|未繳)(?=(?:｜|<))/g,''):html;
};
...
function copyVisibleMonth(){const base=new Date($('calendarDate').value+'T00:00:00'),m=`${base.getFullYear()}-${String(base.getMonth()+1).padStart(2,'0')}`;copyMonth(m)}function copyMonth(m){
  ...
  if(typeof lessonCapacityCanAdd==='function'&&!lessonCapacityCanAdd(sourceLessons.length,'複製整月課程'))return;
  ...
  for(const lesson of sourceLessons){
    ...
    const created=createFreshLessonCopy(lesson,{date:newDate,teacherIds:[...lessonTeacherIds(lesson)]});
    db.lessons.push(created);createdIds.push(created.id);

    existingKeys.add(key);
    added++;
  }

  finishScheduleHistory(history,createdIds);commitScheduleMutation('lesson.copy');
...
/* Keep every visible lesson at its real time, including early-morning classes. */
renderWeek=function(date,f){
  if(matchMedia('(max-width:700px)').matches)return renderMobileWeek(date,f);
  const day=date.getDay(),mon=new Date(date);mon.setDate(date.getDate()-((day+6)%7));
  const days=Array.from({length:7},(_,i)=>{const d=new Date(mon);d.setDate(mon.getDate()+i);return d});
  const weekDates=new Set(days.map(localDate));
  const visibleLessons=window.DanbridgeLessonIndex.between(db.lessons,localDate(days[0]),localDate(days[6])).filter(l=>weekDates.has(l.date)&&lessonMatchesCalendar(l,f));
  ...
  for(let i=0;i<days.length;i++){
    ...
    for(const l of lessons){
      const [sh,sm]=l.start.split(':').map(Number),[eh,em]=l.end.split(':').map(Number),startMin=sh*60+sm,endMin=eh*60+em,startSlot=Math.round((startMin-startHour*60)/5),span=Math.max(1,Math.round((endMin-startMin)/5)),rowStart=startSlot+2,rowEnd=Math.min(totalSlots+2,rowStart+span);
      const durationMinutes=endMin-startMin,durationLabel=durationMinutes%60?`${Math.floor(durationMinutes/60)} 小時 ${durationMinutes%60} 分`:`${durationMinutes/60} 小時`,durationClass=span>=24?' week-event--xl':span>=12?' week-event--long':'';
      h+=`<div class="week-event${durationClass} ${(l.lessonState==='draft'||l.isDraft)?'lesson-draft':''} ${selectedLessonIds.has(l.id)?'selected':''} ${hasTeacherOverlap(l)?'teacher-overlap':''} ${calendarFilterState().search?'calendar-search-hit':''}" title="${esc(lessonHoverText(l))}" draggable="${selectionMode?'false':'true'}" data-id="${l.id}" data-duration="${durationLabel}" style="grid-column:${i+2};grid-row:${rowStart}/${rowEnd};--teacher:${safeCssColor(teacher(l.teacherId).color)};--location-bg:${locationBg(l.location)}"><b><span class="week-event-time">${l.start}–${l.end}</span><span class="week-event-student">${l.isDraft?'<span class="draft-tag">草稿</span> ':''}${esc(student(l.studentId).name)}</span></b><span class="week-event-meta"><span>${esc(lessonTeacherNames(l))}</span>${effectiveCampId(l)?`<span>營隊 ${esc(effectiveCampId(l))}</span>`:''}<span>📍${esc(locationLabel(l))}</span>${hasTeacherOverlap(l)?`<span>⚠ ${esc(lessonTeacherConflictNames(l).join('、'))}</span>`:''}</span></div>`;
    }
  }
  if(reuseGrid){
