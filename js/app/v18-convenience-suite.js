...
(function(){
  ...
  function statusLabel(record){return record?.status==='review'?'需核對':record?.status==='partial'?'部分已收款':record?.status==='collected'?'已收款':record?.notifiedAt?'已通知':record?.copiedAt?'已複製（未確認發送）':'待通知'}
  ...
  function lessonDiff(){
    ...
    const branchName=id=>window.DanbridgeAccess?.branchName?.(id)||branchRecord(id)?.name||id;
  ...
  function enhanceLinePreview(){const modal=$('#v181LineBillingPreview');if(!modal||modal.dataset.salutationReady==='1')return;modal.dataset.salutationReady='1';const row=document.createElement('div');row.className='v181-salutation-row';row.innerHTML='<label>綁定家長姓名<input class="v181-parent-binding" type="text" readonly aria-readonly="true"></label><label>營隊季別<select class="v18211-line-season"><option value="summer">夏令營</option><option value="winter">冬令營</option></select></label><small class="v18211-line-season-note">姓名只讀取學生／家長 CRM 的「家長姓名」；同一家庭識別碼與家長姓名的孩子合併一份帳單；未填識別碼且有多名孩子時須先核對。下方預覽文字可直接修改，只影響本次複製。</small>';modal.querySelector('textarea')?.before(row);const season=$('.v18211-line-season',row);season.addEventListener('change',()=>{modal.dataset.campSeason=activeCampBillingSeason(season.value);modal.querySelector('textarea').value=studentLineBillingText(modal.dataset.studentId,modal.dataset.month,modal.dataset.scope,familyInfo(modal.dataset.studentId).ids,modal.dataset.campSeason)});
  }
  ...
  function setCollectionStatus(keys,status){
    ...
    const review=keys.map(key=>key.split(',')[0]).filter(billingFamilyNeedsReview);
    if(review.length&&!confirm('以下同名家長尚未設定家庭識別碼，請確認孩子確屬同一家庭再繼續：\n'+review.map(id=>billingLineSalutation(id)+'：'+billingFamilyStudents(id).map(s=>s.name||s.id).join('、')).join('\n')))return;
  ...
  function markCopiedNotified(e){const{studentId,month,scope='all',familySignature}=e.detail||{};if(!studentId||!month)return;if(familySignature&&familySignature!==billingFamilyReviewSignature(studentId))return;const{record}=collectionRecord(studentId,month,true,scope);record.copiedAt=now();record.updatedAt=now();saveDB();window.renderSettlement?.()}
