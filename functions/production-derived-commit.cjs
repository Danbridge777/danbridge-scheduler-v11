...
function accessIdentity(value){
 ...
 return JSON.stringify({email:value.email||'',companyId:value.companyId||'',role:value.role||'',active:value.active===true,teacherId:value.teacherId||'',branchIds:[...(value.branchIds||[])].map(String).sort(),canManageSchedule:value.canManageSchedule===true,canMoveSchedule:value.canMoveSchedule===true,hideFinancials:value.hideFinancials===true,canViewBranchFinance:value.canViewBranchFinance===true,scheduleBranchIds:[...(value.scheduleBranchIds||[])].map(String).sort(),readOnly:value.readOnly===true,revision:value.revision??null});
}
