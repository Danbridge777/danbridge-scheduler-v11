...
async function commitProductionDerivedWrites(firestore,writes){
 let committed=0;
 for(let offset=0;offset<writes.length;offset+=400){
  const batch=firestore.batch();
  for(const write of writes.slice(offset,offset+400)){if(write.type==='delete')batch.delete(write.ref);else batch.set(write.ref,write.value,write.options||{merge:false})}
  await batch.commit();committed+=Math.min(400,writes.length-offset);
 }
 return committed;
}
...
exports.productionPublishRoleViews=onCall({region:'asia-east1',serviceAccount:PRODUCTION_SERVICE_ACCOUNT,enforceAppCheck:true,consumeAppCheckToken:true,timeoutSeconds:540,memory:'1GiB',concurrency:4,minInstances:0,maxInstances:10},async request=>{
 try{
  ...
  for(const view of views){
   if(view.kind==='teacher'){desiredTeachers.add(view.email);const current=teacherCurrent.get(view.email)?.data();if(current?.sourceRecordHash!==input.sourceHash||current?.clientHash!==view.clientHash||projection.productionClientDataHash(current?.db)!==view.clientHash)writes.push({type:'set',ref:firestore.doc(`companies/danbridge/teacherViews/${view.email}`),value:{db:view.db,updatedAt:FieldValue.serverTimestamp(),teacherId:view.teacherId,email:view.email,clientHash:view.clientHash,sourceRecordHash:input.sourceHash,release:input.release}})}
   else if(view.kind==='scheduler'){desiredSchedulers.add(view.email);const current=schedulerCurrent.get(view.email)?.data();if(current?.sourceRecordHash!==input.sourceHash||current?.clientHash!==view.clientHash||projection.productionClientDataHash(current?.db)!==view.clientHash)writes.push({type:'set',ref:firestore.doc(`companies/danbridge/schedulerViews/${view.email}`),value:{db:view.db,updatedAt:FieldValue.serverTimestamp(),email:view.email,clientHash:view.clientHash,sourceRecordHash:input.sourceHash,release:input.release}})}
  ...
  for(const meta of lessonMeta){const saved=metaAfterMap.get(meta.lessonId);if(!saved||saved.sourceRecordHash!==input.sourceHash||projection.productionLessonMetaSignature(saved)!==projection.productionLessonMetaSignature(meta.payload))throw new Error(`production lessonMeta 讀回不一致：${meta.lessonId}`)}
