...
async function stagingSchedulerRuntime(){
 if(!stagingSchedulerRuntimePromise)stagingSchedulerRuntimePromise=(async()=>{
   ...
   await derivedDelivery.warm();
   const cached=derivedDelivery.peek(),warmKeys=['lessons','students','makeups'].flatMap(collection=>(cached?.documentsByCollection?.[collection]||[]).map(row=>({collection,recordId:String(row?.id||'')}))).filter(row=>row.recordId).sort((left,right)=>{const a=`${left.collection}/${left.recordId}`,b=`${right.collection}/${right.recordId}`;return a<b?-1:a>b?1:0});
   for(let offset=0;offset<warmKeys.length;offset+=30)await recordBinder.warmBaselines(warmKeys.slice(offset,offset+30));
   console.info('STAGING_V2_BASELINE_WARM',JSON.stringify({state:'verified',recordCount:warmKeys.length,batchCount:Math.ceil(warmKeys.length/30)}));
...
exports.productionTeacherLeaveOperation=onCall({region:'asia-east1',serviceAccount:PRODUCTION_SERVICE_ACCOUNT,enforceAppCheck:true,consumeAppCheckToken:true,timeoutSeconds:60,memory:'512MiB',concurrency:20,minInstances:0,maxInstances:20},async request=>{
 try{
  ...
  const result=await require('./teacher-leave-runtime.cjs').executeTeacherLeave({firestore:runtimeValue.firestore,identity:actor,request:request.data,serverTimestamp:()=>FieldValue.serverTimestamp(),primaryOwnerEmail:PRIMARY_OWNER_EMAIL});
  return{schema:'danbridge-teacher-leave-operation-response-v1',ok:true,...result};
