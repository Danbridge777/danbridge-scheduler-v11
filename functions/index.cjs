...
exports.productionPitrClonePreview=onCall({region:'asia-east1',serviceAccount:PRODUCTION_SERVICE_ACCOUNT,enforceAppCheck:true,consumeAppCheckToken:true,timeoutSeconds:540,memory:'1GiB',concurrency:2,minInstances:0,maxInstances:2},async request=>{
 try{
  ...
  const receiptRef=runtimeValue.firestore.doc(`companies/danbridge/pitrPreviews/${input.databaseId}`),receiptSnapshot=await receiptRef.get(),receipt=receiptSnapshot.exists?receiptSnapshot.data():null;if(!receipt||receipt.createdByUid!==caller.uid||!receipt.operationName)throw new Error('找不到 PITR 暫存還原');
  ...
  if(operationData.done!==true)return{schema:'danbridge-production-pitr-clone-preview-response-v1',state:'cloning',databaseId:input.databaseId,snapshotTime:receipt.snapshotTime.toDate().toISOString(),operationName:receipt.operationName,progress:operationData.metadata?.progressPercentage||{},formalDataWrites:0};
  if(receipt.state==='ready-read-only'&&receipt.diff)return{schema:'danbridge-production-pitr-clone-preview-response-v1',state:'ready-read-only',databaseId:input.databaseId,snapshotTime:receipt.snapshotTime.toDate().toISOString(),diff:receipt.diff,formalDataWrites:0};
  const previewFirestore=getFirestore(runtimeValue.app,input.databaseId),[currentByCollection,previewByCollection]=await Promise.all([productionRecordFingerprints(runtimeValue.firestore,FULL_RECORD_COLLECTIONS),productionRecordFingerprints(previewFirestore,FULL_RECORD_COLLECTIONS)]),diff=buildProductionPitrDiff({databaseId:input.databaseId,snapshotTime:receipt.snapshotTime.toDate().toISOString(),currentByCollection,previewByCollection});
  ...
  return{schema:'danbridge-production-pitr-clone-preview-response-v1',state:'ready-read-only',databaseId:input.databaseId,snapshotTime:receipt.snapshotTime.toDate().toISOString(),diff,formalDataWrites:0};
