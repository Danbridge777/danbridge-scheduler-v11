...
async function executeStagingAuthorityPayload({payload,recordBinder,auditBinder,derivedDelivery,trustedHashes=null}){
 ...
 if(!recordIndexes.length){const auditCompletion=auditPayloads.length===1?await auditBinder.execute(auditPayloads[0]):await auditBinder.executeBatch(auditPayloads),completion=publicCompletion(null,auditCompletion);await derivedDelivery.deliver(payload,completion,trustedHashes);return completion}
 if(!auditIndexes.length){const completion=await recordBinder.execute(payload);await derivedDelivery.deliver(payload,completion,trustedHashes);return completion}
 ...
 const completion=publicCompletion(recordCompletion,auditCompletion);await derivedDelivery.deliver(payload,completion,trustedHashes);return completion;
}
