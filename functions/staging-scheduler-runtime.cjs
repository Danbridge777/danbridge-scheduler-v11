...
async function createStagingSchedulerRuntime({firestore,serverTimestamp,executeAuthorityPayload,getCachedAuthoritySnapshot=()=>null,primaryOwnerEmail,now=()=>Date.now()}={}){
 ...
 const responseFor=async({request,db,schedulerEmail,operationCount=0,evidence=null})=>{let revision=Number(evidence?.sourceRecordRevision),sourceHash=String(evidence?.sourceHash||''),notificationCount=Number(evidence?.notificationCount)||0;if(!Number.isSafeInteger(revision)||revision<1||!/^record-v1:[a-f0-9]{64}$/.test(sourceHash)){const control=await readDocument(`stagingRoleRecordViewControls/danbridge/views/${schedulerEmail}`);revision=Number(control?.revision);sourceHash=String(control?.sourceRecordHash||'')}if(!Number.isSafeInteger(revision)||revision<1||!/^record-v1:[a-f0-9]{64}$/.test(sourceHash))throw new Error('staging 排課專員角色檢視尚未完成新版讀回');return{schema:policy.SCHEDULER_OPERATION_RESPONSE_SCHEMA,requestId:request.requestId,state:'committed',sourceHash,sourceRecordRevision:revision,operationCount,notificationCount,schedulerDb:projection.projectProductionSchedulerDb(db)}};
  ...
  if(!identity||identity.emailVerified!==true||identity.appVerified!==true||typeof identity.email!=='string')throw new Error('排課專員登入驗證無效');
   ...
   console.info('STAGING_SCHEDULER_PREPARED',JSON.stringify({cacheHit:Boolean(cachedValid),elapsedMs:now()-started}));
    ...
    if(plan.operations.length===1)await sender.apply(plan.operations[0]);else await sender.applyBatch(plan.operations);
	    response=await responseFor({request,db:target.db,schedulerEmail:caller.email,operationCount:plan.operationCount,evidence:schedulerEvidence});
	   }
