...
async function executePublishedWorkspace({native,serverTimestamp,deleteField,identity,data,projectId,preserveLegacyViews=false}){
 ...
 const dependencies={firestore,serverTimestamp,deleteField,primaryOwnerEmail:MEMBERS[0],release:'20.26.344',preserveLegacyViews,historyVersionCache:true,onTiming:sample=>console.info('ISOLATED_OWNER_TIMING',JSON.stringify(sample))};
 ...
 if(data.action==='seed'){
  ...
  await publisher.execute({schema:'danbridge-production-role-view-publish-v1',requestId:'workspace-seed-'+data.runId,sourceHash,release:'20.26.344'},identity);
