...
- 將全老師排課專員唯一帳號改為 `aa096662336@gmail.com`；Firestore Rules 同步禁止 Wendy 或其他 Gmail 取得此能力。
- Wendy 自動恢復為純老師：撤除 `canManageSchedule` 與全老師排課快照，重新發布她自己的老師檢視；歷史排課要求與稽核保留，不刪除任何課程。

## 20.26.14

- 修正核准排課專員仍可能因建立帳號時漏勾選而成為一般老師：Wendy 與 `aa096662336@gmail.com` 現在會強制套用相同排課能力。
- Owner 發布角色檢視時會自動修復既有核准排課專員的 `canManageSchedule` 與最新全老師排課資料，不修改任何課程主資料。

## 20.26.13

- 新增 `aa096662336@gmail.com` 為第二個核准排課專員帳號，與 Wendy 共用相同的全老師排課、快速操作、同步佇列及隱私隔離能力。
