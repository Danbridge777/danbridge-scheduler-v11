...
## 20.26.264（CRM 工具列尺寸回讀修正）

- 20.26.263 正式回讀發現老師篩選欄仍為 46px，手機搜尋欄仍受 16px 舊樣式覆蓋；統一為與表單相同的 48px 高、14px 字級及靠左文字。
- 尺寸測試擴大到學生／家長整個頁面的輸入與選單，不再只檢查左側表單；保留 20.26.263 的全部功能修正。
- 更新全部啟用資源指紋及快取世代；不改正式資料、計費、後端或角色規則，40 筆／2 秒／120Hz 仍未達標。

## 20.26.263（精簡家教／團課、登入等待與週課表重畫改善）

- 學生／家長改為家教、團課雙分頁；團課學生搜尋移至表單上方，聯絡與學習資料、計薪說明可展開，保留既有欄位與草稿。
- 表單及按鈕統一 48px 高、14px 字級，文字靠左、選單線條箭頭、精簡邊框；不改學生／家長、團課成員與計費關聯。
- 登入時間紀錄改為非阻塞，必須先完成角色驗證，仍保留 16 集合完整性及安全唯讀檢查；啟動時顯示已讀取集合進度。
- 週課表保留未變更時間格，避免每次操作重建逾千格；保留拖曳事件單次綁定、換週及可編輯權限變更時重建。
- 移除完整資料雜湊前的重複排序，30,000 堂合成資料驗證雜湊相同且原始資料不變。
- 40 筆／2 秒／持續 120Hz 尚未達標。正式保留 30 筆安全上限；本次只部署 Hosting，不部署後端、Rules 或 IAM。

## 20.26.262（課表工作區整理與四帳號權限驗證）

- 課表日期導覽、新增與常用工具分區；篩選與搜尋改為可展開區塊，收起時仍顯示有效條件。
- 課表控制項沿用全站 48px 高度、14px 按鈕字級，窄螢幕自動分行；移除重複新增入口與多層裝飾。
- 移除總覽 AI 裝飾字樣及自動整理說明，改用「待辦事項」；保留真實營運提醒。
- 保留全部既有控制項 ID、課程資料 ID、計費、同步協定、Rules 與角色範圍；增加 Daniel／Catherine／Lucas／AA 的隔離權限實測。
- 40 筆／2 秒實驗未達發布門檻，未納入本版；維持現行 30 筆請求上限，不宣稱 120Hz 或兩秒雲端保證。

## 20.26.261（收款狀態與提醒一致性）

- 修正 staging 真實驗收發現的跨校區收款標示：家庭列使用與摘要相同的逐項對帳結果，已收款、部分已收款與需核對不再互相矛盾。
- 通知中心及總覽的未收款提醒採用同一份收款對帳；團班逐孩分家長、安親月費每月一次，已收款不再被舊課程付款旗標誤判，月份不混算。
- 20.26.260 僅發布 staging 驗收；本版包含該版修正後一起進正式環境。

## 20.26.260（逐項收款對帳與 LINE 複製存檔修正）

- 收款保存孩子、月份、課程與營收歸屬校區明細；同一費用的課程付款、全校區／單校區收款不重複沖銷，也不抵扣其他家庭費用。
- 修正收款確認及 LINE 複製後通知紀錄未存檔的錯誤；剪貼簿失敗不標示成功，提醒不覆寫既有已收款明細。
- 月底檢查依團班實際孩子展開；舊的部分收款缺少分配明細時顯示「需核對」，保留原紀錄不猜配。
- 新收款明細納入角色投影校區／學生隔離；不變更正式課表或學生 ID，不搬移既有資料。

## 20.26.259（家教／團班左右分頁）

- 學生／家長頁面改為左側「家教」、右側「團班」分頁；沿用一致的卡片、表單與按鈕，分開呈現學生與團班清單，安親及個別團班學生仍保留於學生側。
- 團班登記使用班名、可搜尋學生名單、歸屬／上課校區及整班老師鐘點費，不顯示個人家長欄位或額外班級營收費率；Owner 可預覽每位成員收費與家長。
- 分頁切換保留未儲存表單；既有團班 ID、成員關聯、課表名單快照、同步與計費公式不變。

## 20.26.258（團班學生即時搜尋）

- 團班資料及課表本堂名單新增即時搜尋，Owner 可依學生、家長姓名、電話、學校或年級篩選；非 Owner 課表名單只搜尋學生姓名。
- 顯示已選與搜尋結果人數，搜尋不取消隱藏的已選學生；無結果有明確提示，清空搜尋恢復完整名單。
- 不變更學生 ID、名單儲存、計費、薪資或雲端同步協定。

## 20.26.257（兼職逐生收費與整班老師成本）

- 學生原有收費欄位維持營收來源，新增「支付兼職老師的鐘點費」，兩筆費用獨立保存與計算。
- 兼職純時薪老師依每堂學生設定計薪；團班採團班容器的鐘點費，每堂只計一次，不加總孩子成本。留白沿用老師原時薪，明確填 0 不回退。
- 正職、固定底薪、既有營隊去重不改計薪規則；月份、薪資摘要與月結快照保留實際混合費率明細。
- 老師及排課專員角色檢視不暴露學生收費與兼職成本。

## 20.26.256（營收與老師授課校區隔離驗收）

- 收款依營收歸屬校區；老師分校授課時數與薪資明細仍依實際上課校區，兩者不互相改寫。
- 新增跨校區團班回歸：河西營收 2,100、美術營收 0；老師在美術授課 1.5 小時、薪資 750，不因營收歸屬搬移。
- 統一課程必填提示為上課校區，將團班角色隱私測試納入標準測試指令。

## 20.26.255（團班介面與課表營收歸屬）

- 勾選框統一為細線霧金樣式，輸入與選單尺寸一致；團班名單支援整列點選及鍵盤操作。
- 補齊實際月底收款頁的團班成員展開，按孩子及家長分帳，團班容器不重複計費。
- 所有學生新增獨立的歸屬校區、預設上課校區。課程保存營收歸屬快照；舊課未設定時使用學生明確填寫的歸屬校區，絕不以舊 branchIds、教室或上課時數猜填。未設定收入列未歸屬，仍納入公司總額。
- 安親按學生歸屬校區每月只收一次；家教與團班按課表時數及每位孩子鐘點費計算，公司營收不因多位老師重複加總。

## 20.26.254（團班學生名單與逐孩帳單）

- 新增團班入口，可從現有學生勾選名單，以學生 ID 分辨同名孩子，保留每位孩子的家長與鐘點費。
- 每堂團班保存學生名單；複製、移動及系列修改攜帶名單，更改預設名單不重算已排課程。
- 月底依課表時間逐孩計費、按月份及家長分帳，團班容器不另收費；老師時數維持每堂一次。
- 課程詳情與老師回報顯示班內學生；老師資料投影僅提供任課名單，不提供家長聯絡或收費資料。

## 20.26.253（學生表單與計費時間來源統一）

- 移除學生新增／修改表單的「每週可上課時段」輸入，既有歷史欄位保留但不覆寫。
- 家教與團班的應收一律取正式課表每堂開始／結束時間計算；安親維持每月固定月費。

## 20.26.252（依課程類型鎖定計費與家長帳單）

- 安親改為在籍學生每月固定月費；當月排課堂數、時數與上課狀態均不重複增加月費。
- 家教與團班統一為「每位孩子的總時數 × 該孩子鐘點費」；舊的每堂／月費標籤不再覆蓋課程類型規則。
- 同時段團班仍以每個學生 ID 獨立計算，LINE 預覽只依學生 CRM 的家長姓名分帳；不同家長不合併，同一家長手足才合併。
- 儀表板、月底結算、年度／月度 Excel、CSV、收款摘要與月結快照共用相同公式，並新增計費公式版本供歷史核對。
- 新增純計算與 Mac、Windows、iOS、iPad、Android 瀏覽器驗收，鎖定家長姓名、安親月費及團班逐孩金額不變條件。

## 20.26.251（課表首次複製焦點與家長姓名預覽）

- 新增課程選取學生後，依學生 ID 帶入偏好老師、單一校區，以及所選日期前最近有效課程的教室、課程名稱、時長與上課方式；到府地址優先取學生最新資料。保留手動選定日期與開始時間，不帶入已繳費／出席／回報狀態，不覆寫既有課程。
- 框選及選取課程後將鍵盤焦點交回課表，修正搜尋後第一次 Command+C／Ctrl+C 被當成輸入欄位快捷鍵而略過的問題；不改文字欄位複製行為。
- 月／週課程卡、詳情標題、批次調整預覽及學生欄位目前選定值，提供依學生 ID 精確綁定的家長姓名提示；同名學生不混用，未填家長明確顯示，保留老師角色隱私限制。
- 新增首次快捷鍵、重新載入、連續三輪各八堂複製、同名家長、HTML 安全及角色切換隱藏的本機隔離瀏覽器回歸測試。
- 僅更新 Hosting 前端，不更動 Firestore 規則、後端功能、正式資料或薪資計算公式。

## 20.26.250（家庭計費正式發布與老師月薪／請假比例核算）

- 月底結算新增「匯出年度 Excel」，依目前選定年份一次輸出 12 個月總覽、家庭應收、老師薪資、請假／不足扣款，以及全年正式課程逐筆明細；匯出後保留原本選定月份。
- 財務月份選單由固定到 2028 改為至少保留未來十年，並自動涵蓋既有課程資料中的較晚年份，避免長期使用後無法選取或匯出。
- 家庭 LINE 計費預覽與可編輯複製功能進入同一正式發布候選版；家長姓名、手足合併、日期、堂數、時數與金額均依所選月份隔離，不讀取其他月份或歷史稱謂。
- 固定底薪老師的每月最低時數依「每週最低時數 ÷ 固定工作日數 × 當月實際固定工作日數」計算，不再假設每月固定四週。
- 不足時數依老師資料內設定的扣款時薪扣除，超出時數依設定的超時時薪加給；有效老師請假另以「底薪 ÷ 當月最低時數 × 請假時數」按比例扣除。
- 同日重疊請假區間只計一次，取消、其他老師、其他月份及非固定工作日的請假不納入；若請假日同時造成課表時數不足，先抵銷相同不足時數，避免雙重扣款。
- 新增跨月、跨平台真實瀏覽器驗收，確認切換月份時財務、老師 KPI、學生收款、家庭 LINE 與一次性支出月份同步更新。

## 20.26.249（家庭 LINE 計費預覽與跨平台操作修正）

- LINE 費用明細只綁學生／家長 CRM 內輸入的「家長姓名」，不再讀取歷史自訂稱謂；同一正規化家長姓名的 2～3 位孩子合併成同一份家庭帳單，舊 ID 清單亦不能混入其他家庭。
- 一般家教與團班逐堂列出日期、時間與小時數，並顯示上課天數、堂數、總時數、`時數 × 學生單價` 算式、分類小計及月份總計；舊的「每堂／月費」標籤不能覆蓋家教與團班的實際計算。
- 預覽文字可直接修改後再確認複製，修改只影響當次剪貼簿內容、不改正式資料；修正 iPhone、Android 行動版導覽列遮住確認按鈕的層級問題。
- 新增純計費與真實瀏覽器防回歸測試，涵蓋三位手足、其他家庭隔離、一般課、團班、排除項目及 Chrome／Safari／Windows／iPad／iPhone／Android。

## 20.26.248（App Check 一次性權杖職責拆分）

- 將 staging 與 production 排課共用的 limited-use App Check 權杖預取、單次取用、逾時淘汰與失敗恢復，從大型登入同步模組抽成獨立且可單元測試的模組。
- 保留原本 90 秒預取快取、120 秒最大權杖年齡與取用後立即預取下一枚的行為；通知仍逐次取得獨立新權杖，不與排課操作共用。
- 本次只做等價拆分，不變更 Firestore Rules、後端 Functions、資料格式、角色權限、同步順序或 production 資料。
