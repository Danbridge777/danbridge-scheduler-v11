...
test('頁面匯入獨立 record-shadow adapter 且只公開專屬手動入口',()=>{
 assert.match(source,/import \{createFirebaseRecordShadowAdapter\} from '\.\/firebase-record-shadow-adapter\.js\?v=20\.26\.237'/);
...
test('Checkpoint B 以版本化 service worker 解除舊 staging 快取',()=>{
 assert.match(pwaSource,/register\('\.\/sw\.js\?v=20\.26\.237'/);
});

test('候選驗證主模組與內層 adapter 必須載入同一版歷史來源語意',()=>{
 assert.match(source,/createFirebaseFullRecordShadowAdapter\} from '\.\/firebase-full-record-shadow-adapter\.js\?v=20\.26\.237'/);
 assert.match(source,/FULL_RECORD_COLLECTIONS,rebuildFullRecordShadowDb\} from '\.\/cloud-full-record-shadow\.js\?v=20\.26\.237'/);
 assert.match(fullAdapterSource,/verifyFullRecordShadowCandidate\} from '\.\/cloud-full-record-shadow\.js\?v=20\.26\.237'/);
 assert.match(source,/createFirebaseRoleViewCandidateAdapter\} from '\.\/firebase-role-view-candidate-adapter\.js\?v=20\.26\.237'/);
 assert.match(source,/verifyOwnRoleViewCandidateReadback\} from '\.\/cloud-role-view-candidate\.js\?v=20\.26\.237'/);
 assert.match(roleCandidateAdapterSource,/verifyRoleViewCandidateDocuments\} from '\.\/cloud-role-view-candidate\.js\?v=20\.26\.237'/);
});
...
test('staging live 預檢只讀取證據與逐筆現況，不寫入、不接管且不掛入既有同步',()=>{
 assert.match(source,/import \{buildStagingLivePreflight\} from '\.\/cloud-staging-live-preflight\.js\?v=20\.26\.237'/);
