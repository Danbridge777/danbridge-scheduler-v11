...
test('真實本機 Firestore：AA 原子新增、即時移動、刪除、重送、併發、角色通知及中途失敗不留半套', {skip:!process.env.FIRESTORE_EMULATOR_HOST,timeout:180000},async()=>{
 ...
 const phases=[],dependencies={firestore,serverTimestamp:()=>FieldValue.serverTimestamp(),primaryOwnerEmail:'owner@example.com',now:()=>Date.parse('2026-09-03T15:00:00Z'),onTiming:row=>phases.push(row)},runtime=await createProductionSchedulerRuntime(dependencies),timings=[];
 ...
 try{
  ...
  const batchSize=Number(process.env.DANBRIDGE_SCHEDULER_BATCH_SIZE||15),thresholdMs=Number(process.env.DANBRIDGE_SCHEDULER_THRESHOLD_MS||2500);
  assert.ok(Number.isInteger(batchSize)&&batchSize>=1&&batchSize<=40);assert.ok(Number.isInteger(thresholdMs)&&thresholdMs>=1&&thresholdMs<=2500);
  ...
  for(let round=0;round<rounds;round++){
   const rows=independent.map((_,worker)=>Array.from({length:batchSize},(_,i)=>({...lesson(worker+1),id:`bulk-${round}-${worker}-${i}`,date:new Date(Date.UTC(2028,0,1+round*180+i)).toISOString().slice(0,10)})));
   const copies=rows.map(group=>group.map(row=>({...row,id:row.id+'-copy',date:new Date(new Date(row.date+'T00:00:00Z').getTime()+60*86400000).toISOString().slice(0,10)})));
   for(const phase of ['add','move','copy',...(batchSize<=15?['delete']:['delete-original','delete-copy'])]){
    const outcomes=await Promise.allSettled(independent.map(async(worker,index)=>{
     ...
     if(phase==='delete-original')changes=group.map(row=>({lessonId:row.id,before:row,after:null}));
     if(phase==='delete-copy')changes=copy.map(row=>({lessonId:row.id,before:row,after:null}));
  ...
  if(rounds){console.log('ISOLATED_BULK_PERFORMANCE '+JSON.stringify(bulkTimings));assert.equal((await read()).db.lessons.filter(row=>row.id.startsWith('baseline-')).length,baselineLessons);assert.ok(bulkTimings.every(row=>row.ms<=thresholdMs),`Every measured ${batchSize}-lesson concurrent command must finish within ${thresholdMs} ms`);}
 ...
 }finally{if(process.env.DANBRIDGE_SCHEDULER_PROFILE==='1')console.log('ISOLATED_SCHEDULER_PHASES '+JSON.stringify(phases));await firestore.terminate()}
});
