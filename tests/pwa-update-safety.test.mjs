...
for(const flag of ['pending','dirty','inFlight','error'])test(`Lucas ${flag} 時保留移動日誌、不更新`,()=>assert.equal(runtime({branchMoveController:{diagnostics:()=>({[flag]:true})}}).window.__danbridgeCanReloadForUpdate(),false));
test('Lucas 初始化未完成不更新，乾淨已就緒佇列可更新',()=>{
 assert.equal(runtime({branchMoveInit:Promise.resolve()}).window.__danbridgeCanReloadForUpdate(),false);
 assert.equal(runtime({branchMoveInit:Promise.resolve(),branchMoveController:{diagnostics:()=>({})}}).window.__danbridgeCanReloadForUpdate(),true);
});
