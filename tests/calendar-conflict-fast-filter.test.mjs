...
test('actual multi-drag function uses scoped rows and preserves cancel/error atomicity',()=>{
 ...
 for(const outcome of ['allow','student-block','teacher-cancel','throw']){
  ...
  const context=vm.createContext({branchCalendarMoveAllowed:()=>true,db:{lessons},shiftDate:(date,n)=>new Date(Date.parse(date+'T00:00:00Z')+n*86400000).toISOString().slice(0,10),shiftTime:(time,n)=>{const[h,m]=time.split(':').map(Number),v=h*60+m+n;return v<0||v>=1440?'':`${String(Math.floor(v/60)).padStart(2,'0')}:${String(v%60).padStart(2,'0')}`},
