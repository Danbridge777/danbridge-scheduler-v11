...
test('staging profile-only changes cannot reactivate stale financial or scope projections',()=>{
 ...
 const current={role:'branch_manager',teacherId:'t1',branchIds:['art_museum'],scheduleBranchIds:['art_museum','hexi'],hideFinancials:true,canViewBranchFinance:true,active:true};
 for(const patch of [{hideFinancials:false},{canViewBranchFinance:false},{scheduleBranchIds:['art_museum']},{branchIds:['hexi']},{teacherId:'t2'},{role:'teacher'},{canManageSchedule:true}]){
...
function fixture(role='owner'){
 const current={role:'branch_manager',email:'aa@example.test',companyId:'danbridge',active:true,accessRevision:1,branchIds:['art_museum'],hideFinancials:true,canViewBranchFinance:true,canMoveSchedule:false,scheduleBranchIds:['art_museum','hexi']};
...
test('old owner clients cannot silently remove new privacy capabilities; audit reflects effective payload',async()=>{
 ...
 const access=store.get('companyAccess/aa@example.test');assert.equal(access.hideFinancials,true);assert.equal(access.canViewBranchFinance,true);assert.equal(access.canMoveSchedule,false);assert.deepEqual(access.scheduleBranchIds,['art_museum','hexi']);
 assert.equal(store.get('companyAudit/danbridge-retain-capabilities').afterHash,sha256Canonical({...mutation.payload,hideFinancials:true,canViewBranchFinance:true,canMoveSchedule:false,scheduleBranchIds:['art_museum','hexi']}));
});
test('only owner can alter capabilities; malformed flags are rejected without writes',async()=>{
 ...
 for(const patch of [{hideFinancials:'false'},{canViewBranchFinance:'true'},{canMoveSchedule:1},{scheduleBranchIds:['unknown']},{scheduleBranchIds:['hexi','hexi']}]){
