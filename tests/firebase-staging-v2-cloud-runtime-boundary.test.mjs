...
test('Firebase Gen2 的 GCLOUD_PROJECT 單一專案標記可通過且仍精確鎖定 staging',async()=>{const runtimeEnv=env();delete runtimeEnv.GOOGLE_CLOUD_PROJECT;runtimeEnv.GCLOUD_PROJECT=STAGING_V2_PROJECT_ID;const result=await attestStagingV2CloudRuntime({expectedProjectId:STAGING_V2_PROJECT_ID,env:runtimeEnv,authFactory:authFactory()});assert.equal(result.projectId,STAGING_V2_PROJECT_ID);await assert.rejects(()=>attestStagingV2CloudRuntime({expectedProjectId:STAGING_V2_PROJECT_ID,env:{...runtimeEnv,GCLOUD_PROJECT:'production-project'},authFactory:authFactory()}),/:gcloud-project$/);const missing=env();delete missing.GOOGLE_CLOUD_PROJECT;await assert.rejects(()=>attestStagingV2CloudRuntime({expectedProjectId:STAGING_V2_PROJECT_ID,env:missing,authFactory:authFactory()}),/:project-missing$/)});
...
test('拒絕原因只輸出固定非敏感分類，不包含環境值或token',async()=>{await assert.rejects(()=>attestStagingV2CloudRuntime({expectedProjectId:STAGING_V2_PROJECT_ID,env:{...env(),FUNCTION_REGION:'secret-region'},authFactory:authFactory({token:'secret-token-value'})}),new RegExp(`${STAGING_V2_CLOUD_RUNTIME_BLOCKER}:function-region$`));await assert.rejects(()=>attestStagingV2CloudRuntime({expectedProjectId:STAGING_V2_PROJECT_ID,env:env(),authFactory:authFactory({email:'private@example.com'})}),new RegExp(`${STAGING_V2_CLOUD_RUNTIME_BLOCKER}:service-account$`))});
