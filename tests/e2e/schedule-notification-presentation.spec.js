...
test('真實瀏覽器：新通知不打斷新增、移動、刪除，未讀內容仍能手動查看',async({page})=>{
 ...
 <button id="notice" hidden>課表通知</button><button id="own">收到自己的更新</button><button id="other">收到老師的更新</button>
 ...
 const presenter=createScheduleNotificationPresenter({document,button,getActor:()=>({uid:'owner-fixture'}),isBusy:()=>busy||modal.open,render:batch=>{document.getElementById('details').textContent=batch.map(x=>x.id).join(',');modal.showModal()}});
 document.getElementById('own').onclick=()=>{rows.push({id:'own-fixture',createdBy:'owner-fixture'});presenter.update(rows)};
 ...
 await page.getByRole('button',{name:'查看 2 則課表通知'}).click();
 await expect(page.locator('#details')).toHaveText('own-fixture,teacher-fixture');
});
