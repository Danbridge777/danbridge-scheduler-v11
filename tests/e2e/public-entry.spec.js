...
const RELEASE = '20.26.364';
const CLOUD_RELEASE = '20.26.364';
const SCHEDULER_STUDENT_TOOLS_RELEASE = '20.26.364';
const APP_SHELL_RELEASE = '20.26.364';
const BUSINESS_RELEASE = '20.26.364';
const TEACHER_KPI_RELEASE = '20.26.364';
const BRANCH_SCOPE_RELEASE = '20.26.364';
const ROLE_UX_RELEASE = '20.26.364';
const REPORT_STYLE_RELEASE = '20.26.364';
const ROLE_UX_STYLE_RELEASE = '20.26.364';
const PWA_RELEASE = '20.26.364';
const PWA_STYLE_RELEASE = '20.26.364';
const CLEAN_FIELD_RELEASE = '20.26.364';
const LANGUAGE_RELEASE = '20.26.364';
const INTERFACE_CLARITY_STYLE_RELEASE = '20.26.364';
const SCHEDULER_UI_RELEASE = '20.26.364';
const PREMIUM_CONTROLS_RELEASE = '20.26.364';
const PERMANENT_HISTORY_RELEASE = '20.26.364';
const APPLICATION_FEATURES_RELEASE = '20.26.364';
const SCHEDULING_EFFICIENCY_RELEASE = '20.26.364';
const CROSS_PLATFORM_LAYOUT_RELEASE = '20.26.364';
const BILINGUAL_CONTROL_STYLE_RELEASE = '20.26.364';
...
test('永久操作日誌在重新整理後仍可從 IndexedDB 完整讀回',async({page},testInfo)=>{
  ...
  await page.evaluate(async({key,rows})=>{const{createBrowserOperationJournalStorage}=await import('/js/core/browser-operation-journal-storage.js?v=20.26.364'),storage=createBrowserOperationJournalStorage({indexedDB,locks:navigator.locks,key});await storage.save(rows);sessionStorage.setItem('e2eJournalKey',key)},{key,rows});
  await page.reload({waitUntil:'domcontentloaded'});
  const readback=await page.evaluate(async()=>{const key=sessionStorage.getItem('e2eJournalKey'),{createBrowserOperationJournalStorage}=await import('/js/core/browser-operation-journal-storage.js?v=20.26.364');return createBrowserOperationJournalStorage({indexedDB,locks:navigator.locks,key}).load()});
...
test('critical teacher and finance resources load the current release', async ({ page }) => {
  ...
  expect(sources).toContain('./js/modules/teachers/teacher-leave.js?v=20.26.364');
  ...
  expect(styles).toContain('./css/teachers/24-teacher-leave.css?v=20.26.364');
  ...
  expect(styles).toContain(`./css/core/85-v2026349-bilingual-control-consistency.css?v=${BILINGUAL_CONTROL_STYLE_RELEASE}`);
  const manifest = await page.locator('link[rel="manifest"]').getAttribute('href');
  expect(manifest).toBe('./manifest.webmanifest');
  const appleIcon = await page.locator('link[rel="apple-touch-icon"]').getAttribute('href');
  expect(appleIcon).toBe('./icon-192.png?v=20.26.364');
});

test('首頁啟用中的本機 CSS 與 JavaScript 全部使用同一發布指紋',async({page})=>{
  await page.goto('/index.html',{waitUntil:'domcontentloaded'});
  const assets=await page.locator('link[rel="stylesheet"][href],script[src]').evaluateAll(elements=>elements.map(element=>element.getAttribute(element.tagName==='LINK'?'href':'src')).filter(value=>/^(?:\.\/)?(?:css|js)\//.test(value)));
  expect(assets.length).toBeGreaterThan(100);
  expect(assets.filter(value=>!value.endsWith(`?v=${RELEASE}`))).toEqual([]);
});

test('teacher navigation stays readable on light compact layouts in both languages',async({page})=>{
 await page.goto('/index.html',{waitUntil:'load'});
 await page.evaluate(()=>{document.body.classList.remove('auth-locked');document.body.classList.add('teacher-cloud-role');window.DanbridgeAccess.setContext({role:'teacher',email:'teacher@example.com',teacherId:'teacher-1'});window.DanbridgeRoleResponsive.apply()});
 for(const width of [390,768,1024,1100]){
  await page.setViewportSize({width,height:900});
  for(const lang of ['zh','en']){
   await page.evaluate(lang=>window.DanbridgeLanguage.setLanguage(lang),lang);
   const colors=await page.locator('body>nav>button[data-tab]').evaluateAll(buttons=>buttons.filter(b=>getComputedStyle(b).display!=='none').map(b=>({tab:b.dataset.tab,color:getComputedStyle(b).color,background:getComputedStyle(b).backgroundColor,active:b.classList.contains('active')})));
   expect(colors.length).toBeGreaterThan(0);
   for(const color of colors){
    const luminance=value=>value.match(/[\d.]+/g).slice(0,3).map(Number).map(n=>{n/=255;return n<=.04045?n/12.92:((n+.055)/1.055)**2.4}).reduce((sum,n,i)=>sum+n*[.2126,.7152,.0722][i],0);
    const foreground=luminance(color.color),background=luminance(color.background),contrast=(Math.max(foreground,background)+.05)/(Math.min(foreground,background)+.05);
    expect(contrast,JSON.stringify({width,lang,...color})).toBeGreaterThanOrEqual(4.5);
   }
  }
 }
});
...
test('主選單在桌機、平板與手機都只顯示一致文字',async({page})=>{
  const expected=['總覽','學生／家長','老師','書籍代購','請假管理','課表','課程紀錄','補課中心','冬／夏令營','公司財務','備份／iPad','安全設定'];
  await page.goto('/index.html',{waitUntil:'domcontentloaded'});
...
test('老師請假頁在 Daniel、AA、老師三種角色下呈現正確範圍並提交精確時數',async({page})=>{
 ...
 expect(roles.teacher).toEqual({tabs:['dashboard','bookPurchase','teacherLeave','calendar','lessons'],teacherDisabled:true,teacherValue:'teacher-1'});expect(roles.scheduler).toEqual({tabs:['bookPurchase','teacherLeave','calendar'],teacherDisabled:false});
});
...
test('every owner navigation and dashboard shortcut responds before heavy rendering', async ({ page }) => {
  ...
  const result=await page.evaluate(async()=>{
    ...
    const renderNames=['renderDashboard','renderStudents','renderCalendar','renderLessons','renderFinance','renderTeacherLeaves','renderBookPurchases'],rendererByTab={dashboard:'renderDashboard',students:'renderStudents',calendar:'renderCalendar',lessons:'renderLessons',finance:'renderFinance',teacherLeave:'renderTeacherLeaves',bookPurchase:'renderBookPurchases'},originals={},originalFrame=window.requestAnimationFrame,frames=[],renderCalls=[];
    renderNames.forEach(name=>{originals[name]=window[name];window[name]=()=>{renderCalls.push(name)}});
    window.requestAnimationFrame=callback=>{frames.push(callback);return frames.length};
    try{
      const tabs=['dashboard','students','teachers','bookPurchase','teacherLeave','calendar','lessons','makeups','camps','finance','data','security'],checks=[];
  ...
  expect(result.shortcutTargets.sort()).toEqual(['calendar','calendar','calendar','lessons','makeups','teacherLeave'].sort());
});
...
for (const schedulerAccount of [
  ...
  const result = await page.evaluate(schedulerAccount => {
    ...
    return {
      ...
      editingToolsHidden: ['#calendar .calendar-quick-add','#calendar .weekly-copy-btn','#calendar #selectionModeBtn','#calendar .day-add','#courseDrawerEditBtn'].filter(selector => {
...
test('mobile lesson date and all single-line editor controls are contained and centered', async ({ page }) => {
  await page.goto('/index.html', { waitUntil: 'load' });
  const result = await page.evaluate(() => {
    // This is an authenticated-editor layout fixture, not a signed-out modal.
    document.body.classList.remove('auth-locked');
...
test('owner metrics stay centered and compact CRM values align consistently left', async ({ page }) => {
  await page.goto('/index.html', { waitUntil: 'domcontentloaded' });
  const result = await page.evaluate(() => {
  ...
  expect(result.numberAlign).toBe('left');
});
...
test('iPad install action opens usable Safari guidance', async ({ page }, testInfo) => {
  ...
  await page.goto('/index.html', { waitUntil: 'load' });
  // Install action lives in the signed-in header; leave the signed-out gate intact.
  await page.evaluate(() => {
    document.body.classList.remove('auth-locked');
    document.getElementById('authScreen')?.classList.add('hidden');
    document.querySelectorAll('[data-auth-isolated]').forEach(element => {
