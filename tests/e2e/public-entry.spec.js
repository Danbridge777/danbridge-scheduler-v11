...
const RELEASE = '20.26.210';
const CLOUD_RELEASE = '20.26.210';
const SCHEDULER_STUDENT_TOOLS_RELEASE = '20.26.210';
const APP_SHELL_RELEASE = '20.26.210';
const BUSINESS_RELEASE = '20.26.210';
const TEACHER_KPI_RELEASE = '20.26.210';
const BRANCH_SCOPE_RELEASE = '20.26.210';
const ROLE_UX_RELEASE = '20.26.210';
const REPORT_STYLE_RELEASE = '20.26.210';
const ROLE_UX_STYLE_RELEASE = '20.26.210';
const PWA_RELEASE = '20.26.210';
const PWA_STYLE_RELEASE = '20.26.210';
const CLEAN_FIELD_RELEASE = '20.26.210';
const LANGUAGE_RELEASE = '20.26.210';
const INTERFACE_CLARITY_STYLE_RELEASE = '20.26.210';
const SCHEDULER_UI_RELEASE = '20.26.210';
const PREMIUM_CONTROLS_RELEASE = '20.26.210';
const PERMANENT_HISTORY_RELEASE = '20.26.210';
const APPLICATION_FEATURES_RELEASE = '20.26.210';
const SCHEDULING_EFFICIENCY_RELEASE = '20.26.210';
const CROSS_PLATFORM_LAYOUT_RELEASE = '20.26.210';
...
test('永久操作日誌在重新整理後仍可從 IndexedDB 完整讀回',async({page},testInfo)=>{
  ...
  await page.evaluate(async({key,rows})=>{const{createBrowserOperationJournalStorage}=await import('/js/core/browser-operation-journal-storage.js?v=20.26.210'),storage=createBrowserOperationJournalStorage({indexedDB,locks:navigator.locks,key});await storage.save(rows);sessionStorage.setItem('e2eJournalKey',key)},{key,rows});
  await page.reload({waitUntil:'domcontentloaded'});
  const readback=await page.evaluate(async()=>{const key=sessionStorage.getItem('e2eJournalKey'),{createBrowserOperationJournalStorage}=await import('/js/core/browser-operation-journal-storage.js?v=20.26.210');return createBrowserOperationJournalStorage({indexedDB,locks:navigator.locks,key}).load()});
...
test('critical teacher and finance resources load the current release', async ({ page }) => {
  ...
  expect(sources).toContain('./js/modules/teachers/teacher-leave.js?v=20.26.210');
  ...
  expect(styles).toContain('./css/teachers/24-teacher-leave.css?v=20.26.210');
  ...
  expect(appleIcon).toBe('./icon-192.png?v=20.26.210');
});
