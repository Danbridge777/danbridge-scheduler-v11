...
test('真實瀏覽器延遲回條：移動後立即移回、新增後立即刪除均保留最後操作',async({page})=>{
 ...
 <button id="move">移到 B</button><button id="return">移回 A</button><button id="add">新增測試課</button><button id="remove">立即刪除測試課</button>
 ...
 readDocuments:async()=>structuredClone(documents),getLocalDb:()=>structuredClone(ui),applyCloudDb:async next=>{ui=structuredClone(next);render()},
