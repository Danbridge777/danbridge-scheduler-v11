...
test('App Check failure visibly preserves the notice; retry acknowledges without duplicate writes',async({page})=>{
 await page.setContent('<div id="scheduleNotificationModal" data-notification-uid="test_uid" data-notification-recipient-email="test@example.test" data-notification-ids=\'["test_notice"]\'><div class="schedule-notification-dialog"><p>隔離通知</p><div class="schedule-notification-actions"><button id="scheduleNotificationAcknowledge">知道了</button></div></div></div>');
 await page.addScriptTag({content:`let cloudUid='test_uid',cloudEmailKey='test@example.test',attempts=0,writes=0;const auth={currentUser:{uid:cloudUid,email:cloudEmailKey}};function cloudStatus(){};async function scheduleNotificationAcknowledgeCall(){attempts++;if(attempts===1)throw Object.assign(new Error('403'),{code:'appCheck/throttled'});writes++;return{data:{ok:true}}};${handler};document.getElementById('scheduleNotificationAcknowledge').onclick=acknowledgeCurrentScheduleNotification;`});
