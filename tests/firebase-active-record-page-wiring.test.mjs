...
test('同帳號雙分頁登入權限初始化會串行化並使用有限權杖重試',()=>{
 assert.match(source,/import \{loadProfileAfterAuthReady\} from '\.\/cloud-auth-profile-bootstrap\.js\?v=20\.26\.365'/);
...
test('角色候選 manifest 與每位本人收據不可變，URL 只顯示按鈕不會自動寫入',()=>{
 assert.match(source,/cloud-role-view-verification\.js\?v=20\.26\.365/);
...
test('Owner active save 依資料 hash 合併相同意圖，但不同 hash 仍排入下一輪',()=>{
 const queue=block('function queueOwnerCloudSave','function lessonMap');
 assert.match(source,/import \{decideOwnerActiveSaveIntent\} from '\.\/cloud-owner-active-save-intent\.js\?v=20\.26\.365'/);
