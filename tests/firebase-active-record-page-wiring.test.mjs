...
test('同帳號雙分頁登入權限初始化會串行化並使用有限權杖重試',()=>{
 assert.match(source,/import \{loadProfileAfterAuthReady\} from '\.\/cloud-auth-profile-bootstrap\.js\?v=20\.26\.346'/);
...
test('角色候選 manifest 與每位本人收據不可變，URL 只顯示按鈕不會自動寫入',()=>{
 assert.match(source,/cloud-role-view-verification\.js\?v=20\.26\.346/);
...
test('Owner active save 依資料 hash 合併相同意圖，但不同 hash 仍排入下一輪',()=>{
 const queue=block('function queueOwnerCloudSave','function lessonMap');
 assert.match(source,/import \{decideOwnerActiveSaveIntent\} from '\.\/cloud-owner-active-save-intent\.js\?v=20\.26\.346'/);
...
test('Owner 課堂回報直接套用與 listener 套用都先排入逐筆 journal，其他角色不會排 owner core',()=>{
 ...
 assert.match(direct,/const changed=applyReportToLesson\(window\.__danbridgeGetDB\(\)\.lessons\.find\(row=>row.id===lessonId\),report\)/);
 assert.ok(direct.indexOf('await saveTrustedLessonReport')<direct.indexOf('applyReportToLesson('));
...
test('角色逐筆發布重用現有 aa、老師、校區篩選且每個 scope 使用獨立 viewKey',()=>{
 ...
 assert.match(publish,/filteredSchedulerDB\(sourceDb\)/);assert.match(publish,/filteredTeacherDB\(sourceDb,identity\.teacherId\)/);assert.match(publish,/filteredBranchDB\(sourceDb,identity\.branchIds,access\)/);
