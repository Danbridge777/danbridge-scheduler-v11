...
const claims=()=>({aud:STAGING_V2_AUTHORITY_SAVE_PROJECT_ID,iss:`https://securetoken.google.com/${STAGING_V2_AUTHORITY_SAVE_PROJECT_ID}`,sub:uid,user_id:uid,email,email_verified:true,auth_time:Math.floor(now/1000)-86400,iat:Math.floor(now/1000)-10,exp:Math.floor(now/1000)+3590});
...
function setup(overrides={}){let calls=0,appOptions=null;const appClaims={app_id:STAGING_V2_AUTHORITY_SAVE_APP_ID,sub:STAGING_V2_AUTHORITY_SAVE_APP_ID,iss:'https://firebaseappcheck.googleapis.com/883029466360',aud:['projects/883029466360',`projects/${STAGING_V2_AUTHORITY_SAVE_PROJECT_ID}`],iat:Math.floor(now/1000)-10,exp:Math.floor(now/1000)+3600},cfg={expectedProjectId:STAGING_V2_AUTHORITY_SAVE_PROJECT_ID,expectedAppId:STAGING_V2_AUTHORITY_SAVE_APP_ID,allowedOrigins:[...STAGING_V2_AUTHORITY_SAVE_ORIGINS],verifyIdToken:async()=>claims(),verifyAppCheckToken:async(_token,options)=>{appOptions=options;return{appId:STAGING_V2_AUTHORITY_SAVE_APP_ID,token:appClaims,alreadyConsumed:false}},authorizeOwner:async()=>true,executeAuthoritySave:async()=>{calls++;return completion()},now:()=>now,...overrides},gateway=createStagingV2AuthoritySaveHttpGateway(cfg);return{gateway,calls:()=>calls,appOptions:()=>appOptions,appClaims}}
...
test('ID token有效期限、App Check、Owner 與 payload actor 任一不符都不執行寫入',async()=>{const normallyCached=setup({verifyIdToken:async()=>({...claims(),iat:Math.floor(now/1000)-901,exp:Math.floor(now/1000)+2799})});assert.equal((await normallyCached.gateway.handle(raw())).status,200);assert.equal(normallyCached.calls(),1);const cases=[
 {verifyIdToken:async()=>({...claims(),iat:Math.floor(now/1000)-3701,exp:Math.floor(now/1000)+1})},
 {verifyIdToken:async()=>({...claims(),auth_time:Math.floor(now/1000)+31})},
 {verifyIdToken:async()=>({...claims(),auth_time:Math.floor(now/1000),iat:Math.floor(now/1000)-10})},
 {verifyIdToken:async()=>({...claims(),exp:Math.floor(now/1000)+3701})},
 ...
 {verifyAppCheckToken:async()=>({appId:STAGING_V2_AUTHORITY_SAVE_APP_ID,token:{...setup().appClaims,aud:['883029466360',STAGING_V2_AUTHORITY_SAVE_PROJECT_ID]},alreadyConsumed:false})},
