// Explicit, production-only release of an exact emulator-tested artifact.
...
const [directory,mode]=process.argv.slice(2),project='danbridge-d8877';
...
assert.equal(patchProductionNotificationScopeRules(baseline).source,candidate);
...
const before=await readLive();assert.ok([evidence.baseSha256,evidence.candidateSha256].includes(before.sha256),'Production Rules drift');
