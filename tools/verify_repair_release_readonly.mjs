...
for(let i=0;i<paths.length;i+=8)await Promise.all(paths.slice(i,i+8).map(async path=>{
 const response=await fetch(`https://${project}.web.app/${path}?verify=repair-276`),body=Buffer.from(await response.arrayBuffer()),sha256=hash(body),match=response.ok&&sha256===hash(fs.readFileSync(path));assets.push({path,status:response.status,sha256,match});if(!match)process.exitCode=1;
}));
const result={project,dataWrites:0,assets:assets.sort((a,b)=>a.path.localeCompare(b.path))};
