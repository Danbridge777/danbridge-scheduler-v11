...
result.notifications=notices.body.filter(x=>x.document).map(x=>unwrap(x.document.fields)).filter(x=>Array.isArray(x.details)&&x.details.some(d=>ids.has(d.lessonId))).map(x=>({recipient:x.recipientEmail,read:x.read===true,acknowledgedAt:x.acknowledgedAt||null,createdAt:x.createdAt||null,changeCount:x.changeCount,matchedLessonCount:new Set(x.details.filter(d=>ids.has(d.lessonId)).map(d=>d.lessonId)).size,types:[...new Set(x.details.filter(d=>ids.has(d.lessonId)).map(d=>d.type))]}));
