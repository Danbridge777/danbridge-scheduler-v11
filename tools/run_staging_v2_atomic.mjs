...
async function main(){
 ...
 try{
  ...
  const primitives=createStagingV2AdminSupervisorPrimitives({readiness,rules,baselineReadback,gateSourceHashes:artifacts.gateSourceHashes}),wired=createStagingV2AdminSupervisorSteps({registry,primitives}),actions=createStagingV2ConcreteManualSupervisorActions({steps:wired.steps,rollback:wired.rollback}),journal=createFirebaseStagingV2SupervisorJournal({app,firestore,expectedProjectId:PROJECT_ID,runId:runtime.runId}),supervisor=createStagingV2ActivationSupervisor({rawManifest:artifacts.manifest,actions,journal}),result=await supervisor.run(),rows=await journal.readAll();
  const expectedCount=STAGING_V2_SUPERVISOR_PHASES.length*2,atomicRow=rows.find(row=>row.phase==='ATOMIC_ACTIVATION'&&row.event==='completed'),finalRow=rows.find(row=>row.phase==='FINAL_READBACK'&&row.event==='completed');
  if(result.status!=='COMPLETE'||result.lastPhase!=='FINAL_READBACK'||result.atomicStarted!==true||rows.length!==expectedCount||rows.some(row=>row.hostingDeployed!==false||row.productionTouched!==false||row.timeMachineTouched!==false||row.event==='blocked'||row.phase==='ROLLBACK')||!atomicRow||![0,4].includes(atomicRow.writeCount)||!finalRow||finalRow.writeCount!==0||rows.some(row=>row.firestoreRulesDeployed!==false))throw new Error('staging V2 atomic terminal state blocked');
  for(let index=0;index<STAGING_V2_SUPERVISOR_PHASES.length;index++){
   const phase=STAGING_V2_SUPERVISOR_PHASES[index],started=rows[index*2],completed=rows[index*2+1];
