...
assert.match(cloudSource, /const APP_RELEASE='20\.26\.44'/, 'operational errors identify the current deployed release');
...
assert.match(cloudSource, /if\(!schedulerStartupRecoveryChecked\)[\s\S]*schedulerOptimisticLessons\.set\(id,deepCopy\(row\)\)[\s\S]*schedulerBaselineLessons=recoveredAtStartup\?serverBaselineLessons[\s\S]*if\(recoveredAtStartup\)setTimeout\(\(\)=>scheduleSchedulerChanges\(\),0\)/, 'aa reload restores locally saved unsent lessons before accepting the three-lesson server view and immediately resumes upload');
