...
assert.match(cloudSource, /const APP_RELEASE='20\.26\.45'/, 'operational errors identify the current deployed release');
...
assert.match(cloudSource, /function inspectSchedulerLocalRecoveryCandidates\(\)[\s\S]*danbridge_scheduler_v1[\s\S]*danbridge_scheduler_versions_v4[\s\S]*目前僅掃描，尚未還原或上傳任何資料/, 'aa exposes a read-only emergency inventory across primary local data, role cache, drafts, and versions before any recovery write');
