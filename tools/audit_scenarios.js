...
assert.match(cloudSource, /const APP_RELEASE='20\.26\.47'/, 'operational errors identify the current deployed release');
...
assert.match(cloudSource, /confirmed-recovery-\$\{row\.id\}[\s\S]*if\(existing\.exists\(\)\)[\s\S]*operation:'create'[\s\S]*await new Promise\(resolve=>setTimeout\(resolve,250\)\)/, 'confirmed recovery is idempotent and rate-limited through the existing Owner atomic request path');
