...
assert.match(cloudSource, /function acknowledgeCurrentScheduleNotification\(\)[\s\S]*if\(modal\)modal\.hidden=true;[\s\S]*productionNotificationAcknowledgeCall\(\{notificationIds:ids\}\)/, 'acknowledging grouped schedule notifications releases the interface before the protected backend transaction');
...
assert.match(cloudSource, /const APP_RELEASE='20\.26\.157'/, 'operational errors identify the current release');
