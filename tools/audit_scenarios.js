...
const context = {
  ...
  lessonBranchId(lesson) { return String(lesson?.branchId || 'unassigned'); },
  effectiveCampId: () => '',
  sameCampSlot: () => false,
...
assert.match(cloudSource, /function acknowledgeCurrentScheduleNotification\(\)[\s\S]*if\(modal\)modal\.hidden=true;[\s\S]*scheduleNotificationAcknowledgeCall\(\{notificationIds:ids\}\)/, 'acknowledging grouped schedule notifications releases the interface before the environment-specific protected backend transaction');
...
assert.match(cloudSource, /const APP_RELEASE='20\.26\.192'/, 'operational errors identify the current release');
...
assert.match(cloudSource, /createShardedSnapshot\(db,\{hash:dataHash,maxChunkBytes:180000,generationId:'read-only-preflight'\}\)/, 'production health check only performs an in-memory sharding preflight');
...
assert.match(cloudSource,/publishRoleViews:DANBRIDGE_ENVIRONMENT==='production'\?async confirmedDb=>\{[\s\S]*queueScheduleChangeNotifications\(before,confirmedDb,recordDataHash\(confirmedDb\)\);[\s\S]*publishRoleViewsWithRetry\(confirmedDb\)/,'production teacher and aa views and notifications are queued only from the trusted committed per-record plan');
assert.match(cloudSource,/trustCommittedPlan:DANBRIDGE_ENVIRONMENT==='production'/,'production releases the next schedule action from the trusted backend receipt while the stream keeps verifying independently');
...
const extremeBefore={lessons:Array.from({length:1000},(_,index)=>lesson({id:`extreme-${index}`,teacherId:'t1',teacherIds:['t1'],branchId:index%2?'a':'b',start:'10:00'}))};
const extremeAdded={lessons:[...extremeBefore.lessons,...Array.from({length:1000},(_,index)=>lesson({id:`added-extreme-${index}`,teacherId:'t1',teacherIds:['t1'],branchId:index%2?'a':'b'}))]};
const extremeMoved={lessons:extremeAdded.lessons.map(row=>({...row,start:'11:00'}))};
const extremeDeleted={lessons:[]};
const accessRows=[
 {id:'daniel@example.com',email:'daniel@example.com',active:true,role:'owner',displayName:'Daniel'},
 {id:'aa@example.com',email:'aa@example.com',active:true,role:'teacher',teacherId:'aa-teacher',teacherName:'AA',canManageSchedule:true},
 {id:'zhang@example.com',email:'zhang@example.com',active:true,role:'teacher',teacherId:'t1',teacherName:'張毅'},
 {id:'other@example.com',email:'other@example.com',active:true,role:'teacher',teacherId:'t2',teacherName:'無關老師'}
];
for(const [label,from,to,expected] of [
 ['add',extremeBefore,extremeAdded,1000],
 ['move',extremeAdded,extremeMoved,2000],
 ['delete',extremeMoved,extremeDeleted,2000]
]){
 const teacherChanges=context.buildScheduleNotificationChanges(from,to),lessonChanges=context.buildScheduleLessonChanges(from,to);
 const groups=context.buildScheduleNotificationRecipientGroups(accessRows,teacherChanges,lessonChanges,'daniel@example.com','Daniel',new Set(['aa@example.com']));
 const byEmail=new Map(Array.from(groups,row=>[row.recipient.email,row]));
 assert.equal(byEmail.get('daniel@example.com').items.size,expected,`${label}: Owner bell receives every company lesson change exactly once`);
 assert.equal(byEmail.get('aa@example.com').items.size,expected,`${label}: scheduler bell receives every company lesson change exactly once`);
 assert.equal(byEmail.get('zhang@example.com').items.size,expected,`${label}: affected teacher bell receives every own lesson change exactly once`);
 assert.equal(byEmail.get('aa@example.com').recipient.role,'scheduler',`${label}: scheduling authority uses the all-teacher notification scope`);
 assert.equal(byEmail.has('other@example.com'),false,`${label}: unrelated teacher never receives another teacher's lesson details`);
}
...
assert.match(cloudSource, /const alreadyApplied=data\.operation==='delete'\?index<0:index>=0[\s\S]*if\(alreadyApplied\)[\s\S]*status:'applied'[\s\S]*return/, 'duplicate aa requests are acknowledged without rewriting the main document or sending duplicate notifications');
assert.match(cloudSource, /for\(const job of jobs\)[\s\S]*await setDoc\(job\.ref,job\.payload[\s\S]*setTimeout\(\(\)=>scheduleSchedulerChanges\(\),wait\)[\s\S]*schedulerBaselineLessons=\[\.\.\.progress\.values\(\)\]/, 'aa sends large lesson batches sequentially, checkpoints every accepted request, and retries only the unsent remainder');
assert.match(cloudSource, /if\(!schedulerStartupRecoveryChecked\)[\s\S]*schedulerOptimisticLessons\.set\(id,deepCopy\(row\)\)[\s\S]*if\(recoveredAtStartup\)schedulerRecoveryHold=true[\s\S]*schedulerBaselineLessons=recoveredAtStartup\?serverBaselineLessons/, 'aa reload preserves local differences but enters a read-only recovery hold instead of automatically uploading an unconfirmed candidate');
assert.match(cloudSource, /if\(schedulerRecoveryHold\)[\s\S]*救援唯讀鎖定[\s\S]*return/, 'aa recovery hold blocks every scheduler upload until the user confirms the exact recovery source');
assert.match(cloudSource, /function inspectSchedulerLocalRecoveryCandidates\(\)[\s\S]*danbridge_scheduler_v1[\s\S]*danbridge_scheduler_versions_v4/, 'aa exposes a read-only emergency inventory across primary local data, role cache, drafts, and versions before any recovery write');
assert.match(cloudSource, /function buildSchedulerRecoveryDifferenceReport\(\)[\s\S]*missingLessons[\s\S]*missingStudents[\s\S]*readOnly:true/, 'aa can export an exact read-only lesson and student difference without restoring or uploading it');
assert.doesNotMatch(cloudSource, /installOwnerConfirmedRecoveryButton|chooseConfirmedSchedulerRecoveryFile|confirmed-daniel-recovery|CONFIRMED_AA_RECOVERY_SHA256/, 'completed one-time recovery code is fully removed');
...
assert.match(lessonListSource,/const scheduler=document\.body\.classList\.contains\('scheduler-cloud-role'\)[\s\S]*scheduler\?'編輯'/,'Wendy lesson records expose scheduling edit rather than a misleading report action');
...
const schedulerRoleCss=fs.readFileSync(path.join(root,'css/core/79-scheduler-role.css'),'utf8');
assert.match(schedulerRoleCss,/calendar-toolbar-filters\{display:grid!important;grid-template-columns:repeat\(5,minmax\(130px,1fr\)\) minmax\(250px,1\.55fr\)/,'Wendy desktop filters use the exact Owner row geometry');
assert.match(schedulerRoleCss,/--scheduler-card:#fffaf3[\s\S]*main section\[id\]>\.card[\s\S]*background:linear-gradient\(155deg,#fffdf9 0%,var\(--scheduler-card\) 100%\)/,'scheduler workspace cards use the isolated soft orange treatment');
assert.doesNotMatch(schedulerRoleCss,/max-width:700px[\s\S]*calendar-toolbar-filters[\s\S]*grid-template-columns:1fr/,'Wendy no longer has a separate teacher-only mobile toolbar breakpoint');
