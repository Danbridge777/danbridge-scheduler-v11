...
assert.match(cloudSource, /const APP_RELEASE='20\.26\.49'/, 'operational errors identify the current deployed release');
...
assert.match(cloudSource, /CONFIRMED_AA_RECOVERY_SHA256='66b13334[0-9a-f]+'[\s\S]*hash!==CONFIRMED_AA_RECOVERY_SHA256[\s\S]*lessons\.length!==67[\s\S]*new Set\(lessons\.map\(x=>x\.id\)\)\.size!==67/, 'confirmed recovery is pinned to the reviewed file hash, exact count, Daniel scope, date range and unique lesson IDs');
assert.match(cloudSource, /confirmed-recovery-\$\{row\.id\}[\s\S]*permission-denied[\s\S]*if\(exists\)[\s\S]*operation:'create'[\s\S]*await new Promise\(resolve=>setTimeout\(resolve,250\)\)/, 'confirmed recovery tolerates Firestore hiding nonexistent request documents, remains idempotent, and is rate-limited through the Owner atomic path');
assert.match(cloudSource, /function applyConfirmedRecoveryAsOwner\(file\)[\s\S]*runTransaction\(cloud[\s\S]*lessonIds\.has\(lesson\.id\)[\s\S]*transaction\.set\(mainRef[\s\S]*confirmed-recovery-\$\{row\.id\}[\s\S]*status:'applied'/, 'Owner confirmed recovery adds only missing lesson IDs and closes all queued requests in one atomic transaction');
