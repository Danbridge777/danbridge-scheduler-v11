...
assert.match(cloudSource, /const APP_RELEASE='20\.26\.316'/, 'operational errors identify the current release');
...
assert.match(cloudSource,/publishRoleViews:DANBRIDGE_ENVIRONMENT==='production'\?async\(confirmedDb,\{confirmedHash\}=\{\}\)=>\{\s*if\(acceptIsolatedAtomicPublication\(confirmedDb,confirmedHash\)\)return;[\s\S]*queueScheduleChangeNotifications\(before,confirmedDb,recordDataHash\(confirmedDb\)\);[\s\S]*publishRoleViewsWithRetry\(confirmedDb\)/,'production teacher and aa views and notifications retain the committed-plan queue; only verified isolated atomic publication may bypass duplicate work');
