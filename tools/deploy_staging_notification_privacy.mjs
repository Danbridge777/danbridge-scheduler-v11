// Staging only. Compile/re-read exact tested artifact; no business data writes.
...
const before=await readLive();assert.ok([evidence.baseSha256,evidence.candidateSha256].includes(before.sha256),'Rules drift');
const files=[{...before.files[0],content:candidate}],compile=await rules.testRuleset(project,files);
assert.equal((compile.body?.issues||[]).filter(x=>x.severity==='ERROR').length,0,JSON.stringify(compile.body?.issues));
if(mode==='--deploy'&&before.sha256!==evidence.candidateSha256){
 for(const file of ['js/core/firebase-auth-and-cloud-sync.module.js','js/core/schedule-notification-read-scope.js']){
  const r=await fetch(`https://${project}.web.app/${file}?privacy=342`,{cache:'no-store'});assert.equal(r.status,200);
  assert.equal(hash(await r.text()),hash(await readFile(new URL('../'+file,import.meta.url),'utf8')),'Deploy exact frontend first');
 }
